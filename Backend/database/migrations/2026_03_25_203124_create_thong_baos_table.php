    ...
    public function up(): void
    {
        if (!Schema::hasTable('thong_baos')) {
            Schema::create('thong_baos', function (Blueprint $table) {
                ...
                $table->foreignId('moi_gioi_id')->constrained('moi_giois', 'id')->onDelete('cascade');

                // ✅ Người gây ra thông báo (Khách hàng - nullable vì có thể là system notification)
                $table->foreignId('khach_hang_id')->nullable()->constrained('khach_hangs', 'id')->onDelete('set null');

                // ✅ BĐS liên quan
                $table->foreignId('bat_dong_san_id')->constrained('bat_dong_sans', 'id')->onDelete('cascade');
