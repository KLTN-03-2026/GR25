...
function getCoords(coords) {
	...
	} else if (coords.coordinates) return coords.coordinates;
	throw new Error("coords must be GeoJSON Feature, Geometry Object or an Array");
}
function containsNumber(coordinates) {
	if (coordinates.length > 1 && isNumber(coordinates[0]) && isNumber(coordinates[1])) return true;
...
function destination(origin, distance, bearing, options = {}) {
	...
	const lng = radiansToDegrees(longitude1 + Math.atan2(Math.sin(bearingRad) * Math.sin(radians) * Math.cos(latitude1), Math.cos(radians) - Math.sin(latitude1) * Math.sin(latitude2)));
	const lat = radiansToDegrees(latitude2);
	if (coordinates1[2] !== void 0) return point([
		lng,
		lat,
		coordinates1[2]
	], options.properties);
...
//#endregion
//#region node_modules/@turf/along/dist/esm/index.js
function along(line, distance$1, options = {}) {
	const coords = getGeom(line).coordinates;
	let travelled = 0;
	for (let i = 0; i < coords.length; i++) if (distance$1 >= travelled && i === coords.length - 1) break;
	...
	} else travelled += distance(coords[i], coords[i + 1], options);
...
function calculateRhumbBearing(from, to) {
	...
	return (radiansToDegrees(Math.atan2(deltaLambda, deltaPsi)) + 360) % 360;
}
//#endregion
//#region node_modules/@turf/angle/dist/esm/index.js
function angle(startPoint, midPoint, endPoint, options = {}) {
	...
	if (azimuthOB < azimuthOA) azimuthOB = azimuthOB + 360;
	const angleAOB = azimuthOB - azimuthOA;
	if (options.explementary === true) return 360 - angleAOB;
	return angleAOB;
}
//#endregion
//#region node_modules/@turf/meta/dist/esm/index.js
var esm_exports$3 = /* @__PURE__ */ __exportAll({
	coordAll: () => coordAll,
	coordEach: () => coordEach,
...
function propEach(geojson, callback) {
	...
	switch (geojson.type) {
		case "FeatureCollection":
			for (i = 0; i < geojson.features.length; i++) if (callback(geojson.features[i].properties, i) === false) break;
			break;
		case "Feature":
...
function featureEach(geojson, callback) {
	if (geojson.type === "Feature") callback(geojson, 0);
		...
		for (var i = 0; i < geojson.features.length; i++) if (callback(geojson.features[i], i) === false) break;
	}
}
...
function segmentEach(geojson, callback) {
	flattenEach(geojson, function(feature2, featureIndex, multiFeatureIndex) {
		...
		if (coordEach(feature2, function(currentCoord, coordIndex, featureIndexCoord, multiPartIndexCoord, geometryIndex) {
			...
			if (callback(lineString([previousCoords, currentCoord], feature2.properties), featureIndex, multiFeatureIndex, geometryIndex, segmentIndex) === false) return false;
			segmentIndex++;
			previousCoords = currentCoord;
		}) === false) return false;
	});
	...
	flattenEach(geojson, function(feature2, featureIndex, multiFeatureIndex) {
		...
		switch (type) {
			case "LineString":
				...
				break;
			case "Polygon":
				for (var geometryIndex = 0; geometryIndex < coords.length; geometryIndex++) if (callback(lineString(coords[geometryIndex], feature2.properties), featureIndex, multiFeatureIndex, geometryIndex) === false) return false;
				break;
		}
	});
}
...
function findPoint(geojson, options) {
	...
	var coords = geometry.coordinates;
	switch (geometry.type) {
		case "Point": return point(coords, properties, options);
		case "MultiPoint":
			if (multiFeatureIndex < 0) multiFeatureIndex = coords.length + multiFeatureIndex;
...
function calculateArea$1(geom) {
	...
	let i;
	switch (geom.type) {
		case "Polygon": return polygonArea(geom.coordinates);
		case "MultiPolygon":
...
function bboxClip(feature, bbox) {
	...
	switch (type) {
		...
		case "Polygon": return polygon(clipPolygon(coords, bbox), properties);
		case "MultiPolygon": return multiPolygon(coords.map((poly) => {
...
function clipPolygon(rings, bbox) {
	...
	for (const ring of rings) {
		...
		if (clipped.length > 0) {
			...
			if (clipped.length >= 4) outRings.push(clipped);
		}
	}
	return outRings;
}
...
//#endregion
//#region node_modules/@turf/bezier-spline/dist/esm/index.js
var Spline = class {
	constructor(options) {
		this.points = options.points || [];
		this.duration = options.duration || 1e4;
		this.sharpness = options.sharpness || .85;
		this.centers = [];
		this.controls = [];
	...
	*/
	cacheSteps(mindist) {
		...
		for (let t = 0; t < this.duration; t += 10) {
			...
			if (Math.sqrt((step.x - laststep.x) * (step.x - laststep.x) + (step.y - laststep.y) * (step.y - laststep.y) + (step.z - laststep.z) * (step.z - laststep.z)) > mindist) {
				steps.push(t);
				laststep = step;
			}
		}
	...
	*/
	pos(time) {
		...
		return bezier((this.length - 1) * t2 - n, this.points[n], this.controls[n][1], this.controls[n + 1][0], this.points[n + 1]);
	}
};
...
function bezierSpline(line, options = {}) {
	...
	const sharpness = options.sharpness || .85;
	const coords = [];
	const spline = new Spline({
		duration: resolution,
		points: getGeom(line).coordinates.map((pt) => {
			return {
				x: pt[0],
				y: pt[1]
			};
	...
	const pushCoord = (time) => {
		...
		if (Math.floor(time / 100) % 2 === 0) coords.push([pos.x, pos.y]);
	};
	for (var i = 0; i < spline.duration; i += 10) pushCoord(i);
...
//#endregion
//#region node_modules/@turf/boolean-concave/dist/esm/index.js
function booleanConcave(polygon) {
	...
	for (let i = 0; i < n; i++) {
		...
		const zcrossproduct = dx1 * (coords[0][i][1] - coords[0][(i + 1) % n][1]) - dy1 * dx2;
...
//#endregion
//#region node_modules/point-in-polygon-hao/node_modules/robust-predicates/esm/util.js
...
//#endregion
//#region node_modules/point-in-polygon-hao/node_modules/robust-predicates/esm/orient2d.js
...
function orient2dadapt(ax, ay, bx, by, cx, cy, detsum) {
	...
	u$2[2] = _j - (u3 - bvirt) + (_i - bvirt);
	u$2[3] = u3;
	return D[sum$1(C2len, C2, 4, u$2, D) - 1];
}
...
function booleanPointOnLine(pt, line, options = {}) {
	...
	for (let i = 0; i < lineCoords.length - 1; i++) {
		...
		if (isPointOnLineSegment$2(lineCoords[i], lineCoords[i + 1], ptCoords, ignoreBoundary, typeof options.epsilon === "undefined" ? null : options.epsilon)) return true;
	}
	return false;
}
function isPointOnLineSegment$2(lineSegmentStart, lineSegmentEnd, pt, excludeBoundary, epsilon) {
	...
	if (Math.abs(dxl) === Math.abs(dyl) && Math.abs(dxl) === 0) {
		if (excludeBoundary) return false;
		if (pt[0] === lineSegmentStart[0] && pt[1] === lineSegmentStart[1]) return true;
		else return false;
	}
	if (!excludeBoundary) {
...
var require_rbush_min = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(t, i) {
		...
		function t(t, r, e, a, h) {
			(function t(n, r, e, a, h) {
				for (; a > e;) {
					if (a - e > 600) {
						var o = a - e + 1, s = r - e + 1, l = Math.log(o), f = .5 * Math.exp(2 * l / 3), u = .5 * Math.sqrt(l * f * (o - f) / o) * (s - o / 2 < 0 ? -1 : 1);
						t(n, r, Math.max(e, Math.floor(r - s * f / o + u)), Math.min(a, Math.floor(r + (o - s) * f / o + u)), h);
					}
					var p = n[r], d = e, x = a;
		...
		function h(t, i, n, r, e) {
			e || (e = p(null)), e.minX = Infinity, e.minY = Infinity, e.maxX = -Infinity, e.maxY = -Infinity;
			for (var a = i; a < n; a++) {
				var h = t.children[a];
				o(e, t.leaf ? r(h) : h);
			}
		...
		return r.prototype.all = function() {
			...
			for (; r.push(i), !i.leaf && r.length - 1 !== n;) {
				for (var e = Infinity, a = Infinity, h = void 0, o = 0; o < i.children.length; o++) {
			...
			for (var r, e, a, o, s, l, u, m = Infinity, c = Infinity, p = i; p <= n - i; p++) {
...
var import_rbush_min = /* @__PURE__ */ __toESM(require_rbush_min());
function toBBox(geojson) {
	var bbox$5;
	if (geojson.bbox) bbox$5 = geojson.bbox;
	else if (Array.isArray(geojson) && geojson.length === 4) bbox$5 = geojson;
	else if (Array.isArray(geojson) && geojson.length === 6) bbox$5 = [
		geojson[0],
		geojson[1],
		geojson[3],
		geojson[4]
	];
	else if (geojson.type === "Feature") bbox$5 = bbox(geojson);
	else if (geojson.type === "FeatureCollection") bbox$5 = bbox(geojson);
...
var RBush$1 = class {
	constructor(maxEntries = 9) {
		this.tree = new import_rbush_min.default(maxEntries);
	...
	*/
	insert(feature) {
		...
		feature.bbox = feature.bbox ? feature.bbox : bbox(feature);
	...
	*/
	load(features) {
		var load = [];
		if (Array.isArray(features)) features.forEach(function(feature) {
			if (feature.type !== "Feature") throw new Error("invalid features");
			feature.bbox = feature.bbox ? feature.bbox : bbox(feature);
			load.push(feature);
		});
		else featureEach(features, function(feature) {
			if (feature.type !== "Feature") throw new Error("invalid features");
			feature.bbox = feature.bbox ? feature.bbox : bbox(feature);
			load.push(feature);
		});
	...
	*/
	search(geojson) {
		return featureCollection(this.tree.search(toBBox(geojson)));
	}
	/**
	...
	*/
	all() {
		return featureCollection(this.tree.all());
	}
	/**
...
function truncateCoords(coords, factor, coordinates) {
	...
	for (var i = 0; i < coords.length; i++) coords[i] = Math.round(coords[i] * factor) / factor;
	return coords;
}
//#endregion
	...
	if (geometry !== null) {
		switch (geometry.type) {
			case "Polygon":
				...
				break;
			case "LineString": coords = [getCoords(geometry)];
		}
		coords.forEach((coord) => {
			createSegments(coord, geojson.properties).forEach((segment) => {
...
function fillEventQueue(geojson, eventQueue) {
	if (geojson.type === "FeatureCollection") {
		...
		for (let i = 0; i < features.length; i++) processFeature(features[i], eventQueue);
	} else processFeature(geojson, eventQueue);
}
var featureId = 0;
var ringId = 0;
var eventId = 0;
function processFeature(featureOrGeometry, eventQueue) {
	...
	for (let i = 0; i < coords.length; i++) for (let ii = 0; ii < coords[i].length; ii++) {
...
function runCheck(eventQueue, ignoreSelfIntersections) {
	...
	while (eventQueue.length) {
		...
		} else if (event.isLeftEndpoint === false) outQueue.pop();
	}
...
function lineIntersect(line1, line2, options = {}) {
	...
	else if (line1.type === "LineString" || line1.type === "Polygon" || line1.type === "MultiLineString" || line1.type === "MultiPolygon") features.push(feature(line1));
	if (line2.type === "FeatureCollection") features = features.concat(line2.features);
	else if (line2.type === "Feature") features.push(line2);
	else if (line2.type === "LineString" || line2.type === "Polygon" || line2.type === "MultiLineString" || line2.type === "MultiPolygon") features.push(feature(line2));
...
function nearestPointOnLine(lines, inputPoint, options = {}) {
	if (!lines || !inputPoint) throw new Error("lines and inputPoint are required arguments");
	...
	flattenEach(lines, function(line, _featureIndex, lineStringIndex) {
		...
		for (let i = 0; i < coords.length - 1; i++) {
			...
			let intersectPos;
			let wasEnd;
			if (stopPos[0] === inputPos[0] && stopPos[1] === inputPos[1]) [intersectPos, wasEnd] = [stopPos, true];
			else if (startPos[0] === inputPos[0] && startPos[1] === inputPos[1]) [intersectPos, wasEnd] = [startPos, false];
			else [intersectPos, wasEnd] = nearestPointOnSegment(startPos, stopPos, inputPos);
			...
			if (pointDistance < closestPt.properties.pointDistance) {
				...
				closestPt.properties = __spreadProps$1(__spreadValues$4({}, closestPt.properties), {
...
function vectorToLngLat(v) {
	const [x, y, z] = v;
	const lat = radiansToDegrees(Math.asin(Math.min(Math.max(z, -1), 1)));
	return [radiansToDegrees(Math.atan2(y, x)), lat];
}
function nearestPointOnSegment(posA, posB, posC) {
	...
	if (segmentAxis[0] === 0 && segmentAxis[1] === 0 && segmentAxis[2] === 0) if (dot$1(A, B) > 0) return [[...posB], true];
	else return [[...posC], false];
	const targetAxis = cross(segmentAxis, C);
	if (targetAxis[0] === 0 && targetAxis[1] === 0 && targetAxis[2] === 0) return [[...posB], true];
	const I1 = normalize$2(cross(targetAxis, segmentAxis));
	const I2 = [
		-I1[0],
		-I1[1],
		-I1[2]
	];
	const I = dot$1(C, I1) > dot$1(C, I2) ? I1 : I2;
	const segmentAxisNorm = normalize$2(segmentAxis);
	const cmpAI = dot$1(cross(A, I), segmentAxisNorm);
	const cmpIB = dot$1(cross(I, B), segmentAxisNorm);
	if (cmpAI >= 0 && cmpIB >= 0) return [vectorToLngLat(I), false];
	if (dot$1(A, C) > dot$1(B, C)) return [[...posA], false];
	else return [[...posB], true];
}
//#endregion
...
function lineSplit(line, splitter) {
	...
	if (line.type !== "Feature") line = feature(line);
	switch (splitterType) {
		case "Point": return splitLineWithPoint(line, truncatedSplitter);
		case "MultiPoint": return splitLineWithPoints(line, truncatedSplitter);
		case "LineString":
		case "MultiLineString":
		case "Polygon":
		case "MultiPolygon": return splitLineWithPoints(line, lineIntersect(line, truncatedSplitter, { ignoreSelfIntersections: true }));
	}
}
...
function splitLineWithPoint(line, splitter) {
	...
	if (lastCoords.length > 1) results.push(lineString(lastCoords));
	return featureCollection(results);
}
function findClosestFeature(point, lines) {
	...
	featureEach(lines, function(segment) {
		var dist = nearestPointOnLine(segment, point).properties.pointDistance;
...
function booleanContains(feature1, feature2) {
	...
	switch (type1) {
		case "Point": switch (type2) {
			case "Point": return compareCoords$3(coords1, coords2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "MultiPoint": switch (type2) {
			case "Point": return isPointInMultiPoint$1(geom1, geom2);
			case "MultiPoint": return isMultiPointInMultiPoint$1(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "LineString": switch (type2) {
			case "Point": return booleanPointOnLine(geom2, geom1, { ignoreEndVertices: true });
			case "LineString": return isLineOnLine$2(geom1, geom2);
			case "MultiPoint": return isMultiPointOnLine$1(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "Polygon": switch (type2) {
			case "Point": return booleanPointInPolygon(geom2, geom1, { ignoreBoundary: true });
			case "LineString": return isLineInPoly$2(geom1, geom2);
			case "Polygon": return isPolyInPoly$2(geom1, geom2);
			case "MultiPoint": return isMultiPointInPoly$1(geom1, geom2);
			case "MultiPolygon": return isMultiPolyInPoly(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "MultiPolygon": switch (type2) {
			case "Polygon": return isPolygonInMultiPolygon(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		default: throw new Error("feature1 " + type1 + " geometry not supported");
	}
}
function isPolygonInMultiPolygon(multiPolygon, polygon) {
	return multiPolygon.coordinates.some((coords) => isPolyInPoly$2({
		type: "Polygon",
		coordinates: coords
	}, polygon));
}
function isMultiPolyInPoly(polygon, multiPolygon) {
	return multiPolygon.coordinates.every((coords) => isPolyInPoly$2(polygon, {
		type: "Polygon",
		coordinates: coords
	}));
}
function isPointInMultiPoint$1(multiPoint, pt) {
	let i;
	let output = false;
	for (i = 0; i < multiPoint.coordinates.length; i++) if (compareCoords$3(multiPoint.coordinates[i], pt.coordinates)) {
		output = true;
		break;
	}
	return output;
...
function isMultiPointInMultiPoint$1(multiPoint1, multiPoint2) {
	for (const coord2 of multiPoint2.coordinates) {
		...
		for (const coord1 of multiPoint1.coordinates) if (compareCoords$3(coord2, coord1)) {
			matchFound = true;
			break;
		}
...
function isMultiPointOnLine$1(lineString2, multiPoint) {
	...
	for (const coord of multiPoint.coordinates) {
		if (booleanPointOnLine(coord, lineString2, { ignoreEndVertices: true })) haveFoundInteriorPoint = true;
		if (!booleanPointOnLine(coord, lineString2)) return false;
	}
	if (haveFoundInteriorPoint) return true;
	return false;
}
function isMultiPointInPoly$1(polygon, multiPoint) {
	for (const coord of multiPoint.coordinates) if (!booleanPointInPolygon(coord, polygon, { ignoreBoundary: true })) return false;
	return true;
}
function isLineOnLine$2(lineString1, lineString2) {
	...
	for (const coords of lineString2.coordinates) {
		if (booleanPointOnLine({
			type: "Point",
			coordinates: coords
		}, lineString1, { ignoreEndVertices: true })) haveFoundInteriorPoint = true;
		if (!booleanPointOnLine({
			type: "Point",
			coordinates: coords
		}, lineString1, { ignoreEndVertices: false })) return false;
	}
	return haveFoundInteriorPoint;
}
function splitLineIntoSegmentsOnPolygon$1(linestring, polygon) {
	const coords = linestring.coordinates;
	const outputSegments = [];
	for (let i = 0; i < coords.length - 1; i++) {
		const seg = lineString([coords[i], coords[i + 1]]);
		const split = lineSplit(seg, feature(polygon));
		if (split.features.length === 0) outputSegments.push(seg);
		else outputSegments.push(...split.features);
	}
	return featureCollection(outputSegments);
}
function isLineInPoly$2(polygon, linestring) {
	if (!doBBoxOverlap$1(bbox(polygon), bbox(linestring))) return false;
	for (const coord of linestring.coordinates) if (!booleanPointInPolygon(coord, polygon)) return false;
	let isContainedByPolygonBoundary = false;
	const lineSegments = splitLineIntoSegmentsOnPolygon$1(linestring, polygon);
	for (const lineSegment of lineSegments.features) {
		const midpoint = getMidpoint$1(lineSegment.geometry.coordinates[0], lineSegment.geometry.coordinates[1]);
		if (!booleanPointInPolygon(midpoint, polygon)) return false;
		if (!isContainedByPolygonBoundary && booleanPointInPolygon(midpoint, polygon, { ignoreBoundary: true })) isContainedByPolygonBoundary = true;
	}
...
function isPolyInPoly$2(feature1, feature2) {
	...
	if (!doBBoxOverlap$1(bbox(feature1), bbox(feature2))) return false;
	const coords = getGeom(feature2).coordinates;
	for (const ring of coords) for (const coord of ring) if (!booleanPointInPolygon(coord, feature1)) return false;
	return true;
}
...
function compareCoords$3(pair1, pair2) {
...
//#endregion
//#region node_modules/@turf/polygon-to-line/dist/esm/index.js
function polygonToLine(poly, options = {}) {
	const geom = getGeom(poly);
	if (!options.properties && poly.type === "Feature") options.properties = poly.properties;
	switch (geom.type) {
		case "Polygon": return singlePolygonToLine(geom, options);
...
function singlePolygonToLine(poly, options = {}) {
	const coords = getGeom(poly).coordinates;
	return coordsToLine(coords, options.properties ? options.properties : poly.type === "Feature" ? poly.properties : {});
}
function multiPolygonToLine(multiPoly, options = {}) {
	const coords = getGeom(multiPoly).coordinates;
...
var _GeojsonEquality = class _GeojsonEquality {
	...
	compare(g1, g2) {
		...
		if (!sameLength(g1, g2)) return false;
		switch (g1.type) {
			case "Point": return this.compareCoord(g1.coordinates, g2.coordinates);
			case "LineString": return this.compareLine(g1.coordinates, g2.coordinates);
			case "Polygon": return this.comparePolygon(g1, g2);
			case "GeometryCollection": return this.compareGeometryCollection(g1, g2);
			case "Feature": return this.compareFeature(g1, g2);
			case "FeatureCollection": return this.compareFeatureCollection(g1, g2);
			default: if (g1.type.startsWith("Multi")) {
				const g1s = explode$1(g1);
				const g2s = explode$1(g2);
	...
	compareLine(path1, path2, ind = 0, isPoly = false) {
		...
		if (this.direction || sameDirection) return this.comparePath(p1, p2);
		else {
			if (this.compareCoord(p1[ind], p2[p2.length - (1 + ind)])) return this.comparePath(p1.slice().reverse(), p2);
			return false;
		}
	}
	fixStartIndex(sourcePath, targetPath) {
		let correctPath, ind = -1;
		for (let i = 0; i < sourcePath.length; i++) if (this.compareCoord(sourcePath[i], targetPath[0])) {
			ind = i;
			break;
		}
		if (ind >= 0) correctPath = [].concat(sourcePath.slice(ind, sourcePath.length), sourcePath.slice(1, ind + 1));
		return correctPath;
	}
	comparePath(p1, p2) {
	...
	compareFeature(g1, g2) {
		return g1.id === g2.id && (this.compareProperties ? equal$1(g1.properties, g2.properties) : true) && this.compareBBox(g1, g2) && this.compare(g1.geometry, g2.geometry);
	}
...
function geojsonEquality(g1, g2, opts) {
	return new GeojsonEquality(opts).compare(g1, g2);
}
__name(geojsonEquality, "geojsonEquality");
function equal$1(object1, object2) {
	...
	for (var key of objKeys1) {
		...
		const isObjects = isObject$1(value1) && isObject$1(value2);
		if (isObjects && !equal$1(value1, value2) || !isObjects && value1 !== value2) return false;
	}
	return true;
}
__name(equal$1, "equal");
var isObject$1 = /* @__PURE__ */ __name((object) => {
...
function cleanCoords(geojson, options = {}) {
	...
	switch (type) {
		...
		case "MultiPoint":
			...
			break;
		default: throw new Error(type + " geometry not supported");
	}
...
function cleanLine(line, type) {
	...
	while (c < points.length) {
		if (booleanPointOnLine(points[b], lineString([points[a], points[c]]))) b = c;
		else {
	...
	if (type === "Polygon" || type === "MultiPolygon") {
		if (booleanPointOnLine(newPoints[0], lineString([newPoints[1], newPoints[newPoints.length - 2]]))) {
			newPoints.shift();
			newPoints.pop();
			newPoints.push(newPoints[0]);
		}
		if (newPoints.length < 4) throw new Error("invalid polygon, fewer than 4 points");
		if (!equals(newPoints[0], newPoints[newPoints.length - 1])) throw new Error("invalid polygon, first and last points not equal");
	}
	return newPoints;
}
...
function booleanEqual(feature1, feature2, options = {}) {
	...
	precision = precision === void 0 || precision === null || isNaN(precision) ? 6 : precision;
	if (typeof precision !== "number" || !(precision >= 0)) throw new Error("precision must be a positive number");
	if (getGeom(feature1).type !== getGeom(feature2).type) return false;
	return geojsonEquality(cleanCoords(feature1), cleanCoords(feature2), { precision });
}
//#endregion
//#region node_modules/@turf/boolean-crosses/dist/esm/index.js
function booleanCrosses(feature1, feature2) {
	...
	switch (type1) {
		case "MultiPoint": switch (type2) {
			case "LineString": return doMultiPointAndLineStringCross(geom1, geom2);
			case "Polygon": return doesMultiPointCrossPoly(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "LineString": switch (type2) {
			case "MultiPoint": return doMultiPointAndLineStringCross(geom2, geom1);
			case "LineString": return doLineStringsCross(geom1, geom2);
			case "Polygon": return doLineStringAndPolygonCross(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "Polygon": switch (type2) {
			case "MultiPoint": return doesMultiPointCrossPoly(geom2, geom1);
			case "LineString": return doLineStringAndPolygonCross(geom2, geom1);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		default: throw new Error("feature1 " + type1 + " geometry not supported");
	}
...
function doMultiPointAndLineStringCross(multiPoint, lineString) {
	...
	while (i < pointLength && !foundIntPoint && !foundExtPoint) {
		for (var i2 = 0; i2 < lineString.coordinates.length - 1; i2++) {
			...
			if (i2 === 0 || i2 === lineString.coordinates.length - 2) incEndVertices = false;
			if (isPointOnLineSegment$1(lineString.coordinates[i2], lineString.coordinates[i2 + 1], multiPoint.coordinates[i], incEndVertices)) foundIntPoint = true;
			else foundExtPoint = true;
		}
		i++;
	}
...
function doLineStringsCross(lineString1, lineString2) {
	...
	for (const intersectPoint of doLinesIntersect.features) if (!booleanEqual(intersectPoint.geometry, point(lineString1.coordinates[0])) && !booleanEqual(intersectPoint.geometry, point(lineString1.coordinates[lineString1.coordinates.length - 1])) && !booleanEqual(intersectPoint.geometry, point(lineString2.coordinates[0])) && !booleanEqual(intersectPoint.geometry, point(lineString2.coordinates[lineString2.coordinates.length - 1]))) return true;
	return false;
}
function doLineStringAndPolygonCross(lineString, polygon) {
	if (lineIntersect(lineString, polygonToLine(polygon)).features.length > 0) return true;
	return false;
}
function doesMultiPointCrossPoly(multiPoint, polygon) {
	...
	var foundExtPoint = false;
	var pointLength = multiPoint.coordinates.length;
	for (let i = 0; i < pointLength && (!foundIntPoint || !foundExtPoint); i++) if (booleanPointInPolygon(point(multiPoint.coordinates[i]), polygon)) foundIntPoint = true;
	else foundExtPoint = true;
	return foundExtPoint && foundIntPoint;
}
function isPointOnLineSegment$1(lineSegmentStart, lineSegmentEnd, pt, incEnd) {
	...
	if (incEnd) {
		if (Math.abs(dxl) >= Math.abs(dyl)) return dxl > 0 ? lineSegmentStart[0] <= pt[0] && pt[0] <= lineSegmentEnd[0] : lineSegmentEnd[0] <= pt[0] && pt[0] <= lineSegmentStart[0];
...
//#endregion
//#region node_modules/@turf/boolean-disjoint/dist/esm/index.js
function booleanDisjoint(feature1, feature2, { ignoreSelfIntersections = true } = { ignoreSelfIntersections: true }) {
	let bool = true;
...
function disjoint(geom1, geom2, ignoreSelfIntersections) {
	switch (geom1.type) {
		case "Point":
			switch (geom2.type) {
				case "Point": return !compareCoords$2(geom1.coordinates, geom2.coordinates);
				case "LineString": return !isPointOnLine(geom2, geom1);
				case "Polygon": return !booleanPointInPolygon(geom1, geom2);
			}
			break;
		case "LineString":
			switch (geom2.type) {
				case "Point": return !isPointOnLine(geom1, geom2);
				case "LineString": return !isLineOnLine$1(geom1, geom2, ignoreSelfIntersections);
				case "Polygon": return !isLineInPoly$1(geom2, geom1, ignoreSelfIntersections);
			}
			break;
		case "Polygon": switch (geom2.type) {
			case "Point": return !booleanPointInPolygon(geom2, geom1);
			case "LineString": return !isLineInPoly$1(geom1, geom2, ignoreSelfIntersections);
			case "Polygon": return !isPolyInPoly$1(geom2, geom1, ignoreSelfIntersections);
		}
	}
	return false;
}
function isPointOnLine(lineString, pt) {
	for (let i = 0; i < lineString.coordinates.length - 1; i++) if (isPointOnLineSegment(lineString.coordinates[i], lineString.coordinates[i + 1], pt.coordinates)) return true;
	return false;
}
function isLineOnLine$1(lineString1, lineString2, ignoreSelfIntersections) {
	if (lineIntersect(lineString1, lineString2, { ignoreSelfIntersections }).features.length > 0) return true;
	return false;
}
function isLineInPoly$1(polygon, lineString, ignoreSelfIntersections) {
	for (const coord of lineString.coordinates) if (booleanPointInPolygon(coord, polygon)) return true;
	if (lineIntersect(lineString, polygonToLine(polygon), { ignoreSelfIntersections }).features.length > 0) return true;
	return false;
}
function isPolyInPoly$1(feature1, feature2, ignoreSelfIntersections) {
	for (const coord1 of feature1.coordinates[0]) if (booleanPointInPolygon(coord1, feature2)) return true;
	for (const coord2 of feature2.coordinates[0]) if (booleanPointInPolygon(coord2, feature1)) return true;
	if (lineIntersect(polygonToLine(feature1), polygonToLine(feature2), { ignoreSelfIntersections }).features.length > 0) return true;
	return false;
}
function isPointOnLineSegment(lineSegmentStart, lineSegmentEnd, pt) {
	...
	if (dxc * dyl - dyc * dxl !== 0) return false;
	if (Math.abs(dxl) >= Math.abs(dyl)) if (dxl > 0) return lineSegmentStart[0] <= pt[0] && pt[0] <= lineSegmentEnd[0];
	else return lineSegmentEnd[0] <= pt[0] && pt[0] <= lineSegmentStart[0];
	else if (dyl > 0) return lineSegmentStart[1] <= pt[1] && pt[1] <= lineSegmentEnd[1];
	else return lineSegmentEnd[1] <= pt[1] && pt[1] <= lineSegmentStart[1];
}
function compareCoords$2(pair1, pair2) {
	return pair1[0] === pair2[0] && pair1[1] === pair2[1];
}
//#endregion
//#region node_modules/@turf/boolean-intersects/dist/esm/index.js
function booleanIntersects(feature1, feature2, { ignoreSelfIntersections = true } = {}) {
	let bool = false;
	flattenEach(feature1, (flatten1) => {
		flattenEach(feature2, (flatten2) => {
			if (bool === true) return true;
			bool = !booleanDisjoint(flatten1.geometry, flatten2.geometry, { ignoreSelfIntersections });
		});
	});
	return bool;
}
//#endregion
//#region node_modules/@turf/line-overlap/dist/esm/index.js
		...
		featureEach(tree.search(segment), function(match) {
			if (doesOverlaps === false) {
				...
				if ((0, import_fast_deep_equal.default)(coordsSegment, coordsMatch)) {
					doesOverlaps = true;
					if (overlapSegment) overlapSegment = concatSegment(overlapSegment, segment) || overlapSegment;
					else overlapSegment = segment;
				} else if (tolerance === 0 ? booleanPointOnLine(coordsSegment[0], match) && booleanPointOnLine(coordsSegment[1], match) : nearestPointOnLine(match, coordsSegment[0]).properties.pointDistance <= tolerance && nearestPointOnLine(match, coordsSegment[1]).properties.pointDistance <= tolerance) {
					doesOverlaps = true;
					if (overlapSegment) overlapSegment = concatSegment(overlapSegment, segment) || overlapSegment;
					else overlapSegment = segment;
				} else if (tolerance === 0 ? booleanPointOnLine(coordsMatch[0], segment) && booleanPointOnLine(coordsMatch[1], segment) : nearestPointOnLine(segment, coordsMatch[0]).properties.pointDistance <= tolerance && nearestPointOnLine(segment, coordsMatch[1]).properties.pointDistance <= tolerance) if (overlapSegment) {
					const combinedSegment = concatSegment(overlapSegment, match);
					if (combinedSegment) overlapSegment = combinedSegment;
					else additionalSegments.push(match);
...
function concatSegment(line, segment) {
	...
	if ((0, import_fast_deep_equal.default)(coords[0], start)) geom.unshift(coords[1]);
	else if ((0, import_fast_deep_equal.default)(coords[0], end)) geom.push(coords[1]);
	else if ((0, import_fast_deep_equal.default)(coords[1], start)) geom.unshift(coords[0]);
	else if ((0, import_fast_deep_equal.default)(coords[1], end)) geom.push(coords[0]);
	else return;
	return line;
}
//#endregion
//#region node_modules/@turf/boolean-overlap/dist/esm/index.js
function booleanOverlap(feature1, feature2) {
	const geom1 = getGeom(feature1);
	const geom2 = getGeom(feature2);
	const type1 = geom1.type;
	const type2 = geom2.type;
	if (type1 === "MultiPoint" && type2 !== "MultiPoint" || (type1 === "LineString" || type1 === "MultiLineString") && type2 !== "LineString" && type2 !== "MultiLineString" || (type1 === "Polygon" || type1 === "MultiPolygon") && type2 !== "Polygon" && type2 !== "MultiPolygon") throw new Error("features must be of the same type");
	if (type1 === "Point") throw new Error("Point geometry not supported");
	if (geojsonEquality(feature1, feature2, { precision: 6 })) return false;
	let overlap = 0;
	switch (type1) {
		case "MultiPoint":
			for (var i = 0; i < geom1.coordinates.length; i++) for (var j = 0; j < geom2.coordinates.length; j++) {
...
//#endregion
//#region node_modules/@turf/boolean-parallel/dist/esm/index.js
function booleanParallel(line1, line2) {
	if (!line1) throw new Error("line1 is required");
	if (!line2) throw new Error("line2 is required");
	if (getType$1(line1, "line1") !== "LineString") throw new Error("line1 must be a LineString");
	if (getType$1(line2, "line2") !== "LineString") throw new Error("line2 must be a LineString");
...
//#endregion
//#region node_modules/@turf/boolean-touches/dist/esm/index.js
function booleanTouches(feature1, feature2) {
	...
	switch (type1) {
		case "Point": switch (type2) {
			case "LineString": return isPointOnLineEnd(geom1, geom2);
			case "MultiLineString":
				var foundTouchingPoint = false;
				for (var ii = 0; ii < geom2.coordinates.length; ii++) if (isPointOnLineEnd(geom1, {
					type: "LineString",
					coordinates: geom2.coordinates[ii]
				})) foundTouchingPoint = true;
				return foundTouchingPoint;
			case "Polygon":
				for (var i = 0; i < geom2.coordinates.length; i++) if (booleanPointOnLine(geom1, {
					type: "LineString",
					coordinates: geom2.coordinates[i]
				})) return true;
				return false;
			case "MultiPolygon":
				for (var i = 0; i < geom2.coordinates.length; i++) for (var ii = 0; ii < geom2.coordinates[i].length; ii++) if (booleanPointOnLine(geom1, {
					type: "LineString",
			...
			case "LineString":
				...
				for (var i = 0; i < geom1.coordinates.length; i++) {
					...
					if (booleanPointOnLine({
						type: "Point",
						coordinates: geom1.coordinates[i]
					}, geom2, { ignoreEndVertices: true })) return false;
				}
				return foundTouchingPoint;
			case "MultiLineString":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom1.coordinates.length; i++) for (var ii = 0; ii < geom2.coordinates.length; ii++) {
					if (!foundTouchingPoint) {
						if (isPointOnLineEnd({
							type: "Point",
							coordinates: geom1.coordinates[i]
			...
			case "Polygon":
				...
				for (var i = 0; i < geom1.coordinates.length; i++) {
					...
					if (booleanPointInPolygon({
						type: "Point",
						coordinates: geom1.coordinates[i]
					}, geom2, { ignoreBoundary: true })) return false;
				}
				return foundTouchingPoint;
			case "MultiPolygon":
				var foundTouchingPoint = false;
		...
		case "LineString": switch (type2) {
			case "Point": return isPointOnLineEnd(geom2, geom1);
			case "MultiPoint":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom2.coordinates.length; i++) {
					...
					if (booleanPointOnLine({
						type: "Point",
						coordinates: geom2.coordinates[i]
					}, geom1, { ignoreEndVertices: true })) return false;
				}
				return foundTouchingPoint;
			case "LineString":
				var endMatch = false;
				...
				for (var i = 0; i < geom1.coordinates.length; i++) if (booleanPointOnLine({
					type: "Point",
					coordinates: geom1.coordinates[i]
				}, geom2, { ignoreEndVertices: true })) return false;
				return endMatch;
			case "MultiLineString":
				...
				for (var i = 0; i < geom2.coordinates.length; i++) {
					...
					for (var ii = 0; ii < geom1.coordinates[i].length; ii++) if (booleanPointOnLine({
						type: "Point",
			...
			case "MultiPolygon":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom1.coordinates.length; i++) {
					for (var ii = 0; ii < geom2.coordinates.length; ii++) if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
							coordinates: geom1.coordinates[i]
						}, {
		...
		case "MultiLineString": switch (type2) {
			case "Point":
				for (var i = 0; i < geom1.coordinates.length; i++) if (isPointOnLineEnd(geom2, {
					type: "LineString",
					coordinates: geom1.coordinates[i]
				})) return true;
				return false;
			...
			case "LineString":
				...
				for (var i = 0; i < geom1.coordinates.length; i++) {
					...
					for (var ii = 0; ii < geom2.coordinates.length; ii++) if (booleanPointOnLine({
						type: "Point",
						coordinates: geom2.coordinates[ii]
					}, {
						type: "LineString",
			...
			case "MultiLineString":
				...
				for (var i = 0; i < geom1.coordinates.length; i++) for (var ii = 0; ii < geom2.coordinates.length; ii++) {
					...
					for (var iii = 0; iii < geom1.coordinates[i].length; iii++) if (booleanPointOnLine({
						type: "Point",
			...
			case "Polygon":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom1.coordinates.length; i++) for (var ii = 0; ii < geom1.coordinates.length; ii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
			...
			case "MultiPolygon":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom2.coordinates[0].length; i++) for (var ii = 0; ii < geom1.coordinates.length; ii++) for (var iii = 0; iii < geom1.coordinates[ii].length; iii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
		...
		case "Polygon": switch (type2) {
			case "Point":
				for (var i = 0; i < geom1.coordinates.length; i++) if (booleanPointOnLine(geom2, {
					type: "LineString",
					coordinates: geom1.coordinates[i]
				})) return true;
				return false;
			case "MultiPoint":
				...
				for (var i = 0; i < geom2.coordinates.length; i++) {
					...
					if (booleanPointInPolygon({
						type: "Point",
						coordinates: geom2.coordinates[i]
					}, geom1, { ignoreBoundary: true })) return false;
				}
				return foundTouchingPoint;
			case "LineString":
				var foundTouchingPoint = false;
			...
			case "MultiLineString":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom2.coordinates.length; i++) for (var ii = 0; ii < geom2.coordinates[i].length; ii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
							coordinates: geom2.coordinates[i][ii]
			...
			case "MultiPolygon":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom2.coordinates[0].length; i++) for (var ii = 0; ii < geom1.coordinates[0].length; ii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
		...
		case "MultiPolygon": switch (type2) {
			case "Point":
				for (var i = 0; i < geom1.coordinates[0].length; i++) if (booleanPointOnLine(geom2, {
					type: "LineString",
					coordinates: geom1.coordinates[0][i]
				})) return true;
				return false;
			case "MultiPoint":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom1.coordinates[0].length; i++) for (var ii = 0; ii < geom2.coordinates.length; ii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
							coordinates: geom2.coordinates[ii]
			...
			case "MultiLineString":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom1.coordinates.length; i++) for (var ii = 0; ii < geom2.coordinates.length; ii++) for (var iii = 0; iii < geom2.coordinates[ii].length; iii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
			...
			case "Polygon":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom1.coordinates[0].length; i++) for (var ii = 0; ii < geom1.coordinates[0][i].length; ii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
			...
			case "MultiPolygon":
				var foundTouchingPoint = false;
				for (var i = 0; i < geom1.coordinates[0].length; i++) for (var ii = 0; ii < geom2.coordinates[0].length; ii++) for (var iii = 0; iii < geom1.coordinates[0].length; iii++) {
					if (!foundTouchingPoint) {
						if (booleanPointOnLine({
							type: "Point",
...
function isPointOnLineEnd(point, line) {
	if (compareCoords$1(line.coordinates[0], point.coordinates)) return true;
	if (compareCoords$1(line.coordinates[line.coordinates.length - 1], point.coordinates)) return true;
	return false;
}
...
//#endregion
//#region node_modules/@turf/boolean-valid/dist/esm/index.js
function booleanValid(feature) {
	...
	switch (type) {
		case "Point": return coords.length > 1;
		case "MultiPoint":
			for (var i = 0; i < coords.length; i++) if (coords[i].length < 2) return false;
			return true;
		case "LineString":
...
function checkRingsForSpikesPunctures(geom) {
	for (var i = 0; i < geom.length - 1; i++) {
		var point = geom[i];
		for (var ii = i + 1; ii < geom.length - 2; ii++) if (booleanPointOnLine(point, lineString([geom[ii], geom[ii + 1]]))) return true;
	}
	return false;
}
function checkPolygonAgainstOthers(poly, geom, index) {
	var polyToCheck = polygon(poly);
	for (var i = index + 1; i < geom.length; i++) if (!booleanDisjoint(polyToCheck, polygon(geom[i]))) {
		if (booleanCrosses(polyToCheck, lineString(geom[i][0]))) return false;
	}
	return true;
}
//#endregion
//#region node_modules/@turf/boolean-within/dist/esm/index.js
function booleanWithin(feature1, feature2) {
	...
	switch (type1) {
		case "Point": switch (type2) {
			case "MultiPoint": return isPointInMultiPoint(geom1, geom2);
			case "LineString": return booleanPointOnLine(geom1, geom2, { ignoreEndVertices: true });
			case "Polygon":
			case "MultiPolygon": return booleanPointInPolygon(geom1, geom2, { ignoreBoundary: true });
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "MultiPoint": switch (type2) {
			case "MultiPoint": return isMultiPointInMultiPoint(geom1, geom2);
			case "LineString": return isMultiPointOnLine(geom1, geom2);
			case "Polygon":
			case "MultiPolygon": return isMultiPointInPoly(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "LineString": switch (type2) {
			case "LineString": return isLineOnLine(geom1, geom2);
			case "Polygon":
			case "MultiPolygon": return isLineInPoly(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		case "Polygon": switch (type2) {
			case "Polygon":
			case "MultiPolygon": return isPolyInPoly(geom1, geom2);
			default: throw new Error("feature2 " + type2 + " geometry not supported");
		}
		default: throw new Error("feature1 " + type1 + " geometry not supported");
	}
...
function isPointInMultiPoint(point, multiPoint) {
	var i;
	var output = false;
	for (i = 0; i < multiPoint.coordinates.length; i++) if (compareCoords(multiPoint.coordinates[i], point.coordinates)) {
		output = true;
		break;
	}
	return output;
...
function isMultiPointInMultiPoint(multiPoint1, multiPoint2) {
	for (var i = 0; i < multiPoint1.coordinates.length; i++) {
		var anyMatch = false;
		for (var i2 = 0; i2 < multiPoint2.coordinates.length; i2++) if (compareCoords(multiPoint1.coordinates[i], multiPoint2.coordinates[i2])) anyMatch = true;
...
function isMultiPointOnLine(multiPoint, lineString2) {
	...
	for (var i = 0; i < multiPoint.coordinates.length; i++) {
		if (!booleanPointOnLine(multiPoint.coordinates[i], lineString2)) return false;
		if (!foundInsidePoint) foundInsidePoint = booleanPointOnLine(multiPoint.coordinates[i], lineString2, { ignoreEndVertices: true });
	}
...
function isMultiPointInPoly(multiPoint, polygon) {
	...
	for (var i = 0; i < multiPoint.coordinates.length; i++) {
		...
		if (!oneInside) isInside = booleanPointInPolygon(multiPoint.coordinates[i], polygon, { ignoreBoundary: true });
	}
	return output && isInside;
}
function isLineOnLine(lineString1, lineString2) {
	for (var i = 0; i < lineString1.coordinates.length; i++) if (!booleanPointOnLine(lineString1.coordinates[i], lineString2)) return false;
	return true;
}
...
function isLineInPoly(linestring, polygon) {
	if (!doBBoxOverlap(bbox(polygon), bbox(linestring))) return false;
	for (const coord of linestring.coordinates) if (!booleanPointInPolygon(coord, polygon)) return false;
	let isContainedByPolygonBoundary = false;
...
function isPolyInPoly(geometry1, geometry2) {
	...
	if (!doBBoxOverlap(bbox(geometry2), poly1Bbox)) return false;
	for (var i = 0; i < geometry1.coordinates[0].length; i++) if (!booleanPointInPolygon(geometry1.coordinates[0][i], geometry2)) return false;
	return true;
}
...
//#endregion
//#region node_modules/@turf/center/dist/esm/index.js
function center(geojson, options = {}) {
	const ext = bbox(geojson);
	return point([(ext[0] + ext[2]) / 2, (ext[1] + ext[3]) / 2], options.properties, options);
}
//#endregion
//#region node_modules/d3-geo/src/adder.js
var import_jsts_min = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(t, e) {
		...
		var p = function(t) {
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i, [t])).name = Object.keys({ Exception: i })[0], r;
			}
			return l(i, t), s(i, [{
				key: "toString",
				value: function() {
			...
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i, [t])).name = Object.keys({ IllegalArgumentException: i })[0], r;
			}
			return l(i, t), s(i);
		}(p), k = function() {
			return s((function t() {
		...
		A.NaN = NaN, A.isNaN = function(t) {
			...
			if (t === Infinity) return new M(u = 2146435072 | a, l = 0);
			...
			var e, n, i, r, s = t.high, a = t.low, o = s & 1 << 31 ? -1 : 1;
		...
		var D = function(t) {
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i, [t])).name = Object.keys({ RuntimeException: i })[0], r;
			}
			return l(i, t), s(i);
		}(p), F = function(t) {
			function i() {
		...
		}(), q = /* @__PURE__ */ new ArrayBuffer(8), Y = new Float64Array(q), z = new Int32Array(q), X = function() {
			function t() {
				n(this, t), t.constructor_.apply(this, arguments);
			}
			return s(t, [
					...
					value: function(e, n) {
						switch (e) {
							...
							case t.Z:
								this.setZ(n);
								break;
							default: throw new m("Invalid ordinate index: " + e);
						}
					}
				},
				{
		...
		X.DimensionalComparator = function() {
			function t() {
				n(this, t), t.constructor_.apply(this, arguments);
			}
			return s(t, [{
		...
		}(), X.NULL_ORDINATE = A.NaN, X.X = 0, X.Y = 1, X.Z = 2, X.M = 3;
		var U = function() {
			function t() {
				n(this, t), t.constructor_.apply(this, arguments);
			}
		...
		var Z = function() {
			...
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i, [t])).name = Object.keys({ NoSuchElementException: i })[0], r;
			}
			return l(i, t), s(i);
		}(p), W = function(t) {
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i, [t])).name = Object.keys({ UnsupportedOperationException: i })[0], r;
			}
			return l(i, t), s(i);
		}(p), K = function(t) {
			function i() {
			...
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i)).map = /* @__PURE__ */ new Map(), t instanceof Z && r.addAll(t), r;
			}
			return l(i, t), s(i, [
				{
					key: "contains",
		...
		var tt = function(t) {
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i, [t])).name = Object.keys({ EmptyStackException: i })[0], r;
			}
			return l(i, t), s(i);
		}(p), et = function(t) {
			function i(t) {
				var r;
				return n(this, i), (r = e(this, i, [t])).name = Object.keys({ IndexOutOfBoundsException: i })[0], r;
			}
			return l(i, t), s(i);
		}(p), nt = function(t) {
			function i() {
		...
		var st = function() {
			...
			return s(t, [
				...
				},
				{
					...
					value: function() {
						if (1 === arguments.length) {
							if ("number" == typeof arguments[0]) this._hi = arguments[0], this._lo = 0;
							else if (arguments[0] instanceof t) {
								var n = arguments[0];
		...
		var ft = function() {
			return s((function t() {
				...
				},
				{
					...
					value: function() {
						this._minDe = this._minDe.getNode().getEdges().getRightmostEdge(), this._minDe.isForward() || (this._minDe = this._minDe.getSym(), this._minIndex = this._minDe.getEdge().getCoordinates().length - 1);
					}
				},
				{
					key: "findEdge",
			...
			function i(t, r) {
				var s;
				return n(this, i), (s = e(this, i, [r ? t + " [ " + r + " ]" : t])).pt = r ? new X(r) : void 0, s.name = Object.keys({ TopologyException: i })[0], s;
			}
			return l(i, t), s(i, [{
				key: "getCoordinate",
				value: function() {
		...
		var Bt = function(t) {
			...
			return l(i, t), s(i, [
					...
					value: function(t, e) {
						switch (t) {
							...
							case i.Y:
								this.y = e;
								break;
							default: throw new m("Invalid ordinate index: " + t);
						}
					}
				},
				{
				...
				},
				{
					...
					value: function(t) {
						switch (t) {
							case i.X: return this.x;
							case i.Y: return this.y;
						}
		...
		var Vt = function(t) {
			...
			return l(i, t), s(i, [
				...
				},
				{
					...
					value: function(t) {
						switch (t) {
							case X.X: return this.x;
							case X.Y: return this.y;
							case X.Z: return this.getZ();
							case X.M: return this.getM();
						}
						throw new m("Invalid ordinate index: " + t);
					}
				},
		...
		var Jt = function() {
			...
			return s(t, [
				...
				},
				{
					...
					value: function(t, e) {
						switch (e) {
							case ht.X: return this._coordinates[t].x;
							case ht.Y: return this._coordinates[t].y;
		...
		var te = function(t) {
			...
			function i() {
				var t;
				return n(this, i), (t = e(this, i)).map = /* @__PURE__ */ new Map(), t;
			}
			return l(i, t), s(i, [
				{
					key: "get",
			...
			return s(t, [
				{
					...
					value: function(t) {
						switch (t) {
							...
							default: throw new m("Invalid dimension: " + t);
						}
					}
				},
				{
					key: "toGeometry",
					value: function(t) {
						return t.isNull() ? this.createPoint() : t.getMinX() === t.getMaxX() && t.getMinY() === t.getMaxY() ? this.createPoint(new X(t.getMinX(), t.getMinY())) : t.getMinX() === t.getMaxX() || t.getMinY() === t.getMaxY() ? this.createLineString([new X(t.getMinX(), t.getMinY()), new X(t.getMaxX(), t.getMaxY())]) : this.createPolygon(this.createLinearRing([
					...
					value: function() {
						...
						if (this.match(ge)) {
							...
							switch (s) {
								...
								default: throw new Error("Invalid geometry type: " + s);
							}
						}
						throw new Error(this.formatErrorMessage_());
					}
		...
		var Le = function() {
			return s((function t(e) {
				...
				key: "read",
				value: function(t) {
					return new xe(new ke(t), this.geometryFactory).parse();
				}
			}, {
				key: "write",
				value: function(t) {
		...
		var we = function(t) {
			...
			return s((function t() {
				...
				value: function() {
					if (this._startDe = null, this._maxNodeDegree = -1, this._edges = new yt(), this._pts = new yt(), this._label = new Ae(H.NONE), this._ring = null, this._isHole = null, this._shell = null, this._holes = new yt(), this._geometryFactory = null, 0 === arguments.length);
						...
						var t = arguments[0];
						this._geometryFactory = arguments[1], this.computePoints(t), this.computeRing();
					}
				}
			}]);
		}(), De = function(t) {
			...
			return s((function t() {
				...
				value: function() {
					...
					else if (1 === arguments.length) this._label = arguments[0];
				}
			}]);
		}(), qe = function(t) {
			function i() {
		...
		var We = function() {
			...
			return s(t, [
				...
				key: "constructor_",
				value: function() {
					if (this._edge = null, this._label = null, this._node = null, this._p0 = null, this._p1 = null, this._dx = null, this._dy = null, this._quadrant = null, 1 === arguments.length) this._edge = arguments[0];
					else if (3 === arguments.length) {
						var n = arguments[0], i = arguments[1], r = arguments[2];
			...
			return s(t, [
				...
				value: function() {
					...
					this._geometryFactory = arguments[0];
				}
			}, {
			...
			return s((function t() {
				...
				value: function() {
					...
					else if (1 === arguments.length) this._level = arguments[0];
				}
			}]);
		}(), an = {
		...
		var vn = function() {
			...
			return s(t, null, [
				...
				},
				{
					...
					value: function(e, n, i) {
						...
						switch (e) {
							case 0: return t.compareValue(r, s);
							case 1: return t.compareValue(s, r);
							case 2: return t.compareValue(s, -r);
							case 3: return t.compareValue(-r, s);
							case 4: return t.compareValue(-r, -s);
							case 5: return t.compareValue(-s, -r);
							case 6: return t.compareValue(-s, r);
							case 7: return t.compareValue(r, -s);
						}
			...
			return s((function t() {
				...
				},
				{
					key: "createSplitEdge",
					value: function(t, e) {
						return new xn(this.createSplitEdgePts(t, e), this._edge.getData());
					}
				},
				{
					key: "add",
		...
		var wn = function() {
			...
			return s(t, [
				...
				},
				{
					key: "isConcave",
					value: function(t, e, n) {
						return ct.index(t, e, n) === this._angleOrientation;
					}
				},
				{
					key: "simplify",
		...
		var Mn = function() {
			...
			return s(t, [
				...
				},
				{
					...
					value: function(t, e, n, i) {
						var r = this._seg0.p1, s = bn.angle(r, this._seg0.p0), a = bn.angleBetweenOriented(this._seg0.p0, r, this._seg1.p1) / 2, o = bn.normalize(s + a), u = bn.normalize(o + Math.PI), l = i * n, h = n - l * Math.abs(Math.sin(a)), v = new In(r, new X(r.x + l * Math.cos(u), r.y + l * Math.sin(u))), y = v.pointAlongOffset(1, h), d = v.pointAlongOffset(1, -h);
					...
					value: function(t, e) {
						switch (this._bufParams.getEndCapStyle()) {
							case _.CAP_ROUND:
								e.createCircle(t);
								break;
							case _.CAP_SQUARE: e.createSquare(t);
						}
					}
				},
				{
		...
		var Fn = function(t) {
			...
			return s(t, [
				...
				},
				{
					key: "circumcentreDD",
					value: function(t, e, n) {
						var i = ut.valueOf(t.x).subtract(n.x), r = ut.valueOf(t.y).subtract(n.y), s = ut.valueOf(e.x).subtract(n.x), a = ut.valueOf(e.y).subtract(n.y), o = ut.determinant(i, r, s, a).multiply(2), u = i.sqr().add(r.sqr()), l = s.sqr().add(a.sqr()), h = ut.determinant(r, u, a, l), c = ut.determinant(i, u, s, l);
						return new X(ut.valueOf(n.x).subtract(h.divide(o)).doubleValue(), ut.valueOf(n.y).add(c.divide(o)).doubleValue());
					}
				},
				{
					key: "isAcute",
				...
				},
				{
					...
					value: function(t, e) {
						...
						return new Gn(new Gn(t.x + n / 2, t.y + i / 2, 1), new Gn(t.x - i + n / 2, t.y + n + i / 2, 1));
					}
				},
				{
					key: "angleBisector",
				...
				},
				{
					key: "area3D",
					value: function(t, e, n) {
						var i = e.x - t.x, r = e.y - t.y, s = e.getZ() - t.getZ(), a = n.x - t.x, o = n.y - t.y, u = n.getZ() - t.getZ(), l = r * u - s * o, h = s * a - i * u, c = i * o - r * a, f = l * l + h * h + c * c;
				...
				},
				{
					key: "centroid",
					value: function(t, e, n) {
						return new X((t.x + e.x + n.x) / 3, (t.y + e.y + n.y) / 3);
					}
				},
				{
					key: "inCentre",
					value: function(t, e, n) {
						var i = e.distance(n), r = t.distance(n), s = t.distance(e), a = i + r + s;
						return new X((i * t.x + r * e.x + s * n.x) / a, (i * t.y + r * e.y + s * n.y) / a);
					}
				}
			]);
		}(), Yn = function() {
			...
			return s((function t() {
				...
				value: function() {
					...
					this.startIndex = new ei().getChainStartIndices(this.pts);
				}
			}]);
		}(), ii = function() {
			function t() {
			...
			return s(t, [
				...
				},
				{
					...
					value: function(t, e) {
						...
						var r = new Yn(t, e, new An(n, this._bufParams)).getCurves();
		...
		var fi = function() {
			return s((function t() {
				...
				value: function() {
					...
					this._li = arguments[0], this._interiorIntersections = new yt();
				}
			}]);
		}(), gi = function() {
			return s((function t() {
			...
			}), [
				{
					...
					value: function(t) {
						var n = new oi(xn.getNodedSubstrings(t));
						try {
							n.checkValid();
						} catch (t) {
				...
				},
				{
					...
					value: function(t) {
						for (var e = t.iterator(); e.hasNext();) {
							var i = new ui(e.next(), this._scaleFactor, this._li);
			...
			return s(t, [
				...
				},
				{
					...
					value: function() {
						...
						if (1 === arguments.length) {
							var n = arguments[0], r = new ie(t.precisionScaleFactor(this._argGeom, this._distance, n));
				...
				},
				{
					...
					value: function() {
						try {
							this._resultGeometry = new si(this._bufParams).buffer(this._argGeom, this._distance);
						} catch (t) {
			...
			], [
				{
					key: "constructor_",
					value: function() {
						if (this._argGeom = null, this._distance = null, this._bufParams = new _(), this._resultGeometry = null, this._saveException = null, 1 === arguments.length) this._argGeom = arguments[0];
						else if (2 === arguments.length) {
							var e = arguments[0], n = arguments[1];
		...
		var yi = [
			...
			Feature: function(t) {
				...
				if (t.geometry) {
					if (!_i[t.geometry.type]) throw new Error("Unknown GeoJSON type: " + t.type);
...
//#endregion
//#region node_modules/d3-geo/src/clip/polygon.js
...
function polygon_default(segments, compareIntersection, startInside, interpolate, stream) {
...
function merge_default$1(arrays) {
...
function clipExtent(x0, y0, x1, y1) {
	...
	return function(stream) {
		...
		function polygonEnd() {
			var startInside = polygonInside(), cleanInside = clean && startInside, visible = (segments = merge_default$1(segments)).length;
			if (cleanInside || visible) {
				...
				if (visible) polygon_default(segments, compareIntersection, startInside, interpolate, stream);
				stream.polygonEnd();
			}
...
function polygonContains_default(polygon, point) {
	...
	return (angle < -1e-6 || angle < 1e-6 && sum < -1e-6) ^ winding & 1;
}
adder_default();
//#endregion
...
var x0$1 = Infinity, y0$1 = x0$1, x1 = -x0$1, y1 = x1;
var boundsStream = {
	point: boundsPoint,
	lineStart: noop,
	lineEnd: noop,
	...
	return function(rotate, sink) {
		...
		var clip = {
			...
			polygonEnd: function() {
				...
				segments = merge_default$1(segments);
	...
	return {
		...
		point: function(lambda1, phi1) {
			...
			if (abs(delta - pi) < 1e-6) {
				...
				if (abs(lambda0 - sign0) < 1e-6) lambda0 -= sign0 * epsilon$1;
				if (abs(lambda1 - sign1) < 1e-6) lambda1 -= sign1 * epsilon$1;
...
function circle_default(radius, delta) {
	...
	function intersect(a, b, two) {
		...
		var delta = lambda1 - lambda0, polar = abs(delta - pi) < epsilon$1, meridian = polar || delta < 1e-6;
...
function projectionMutator(projectAt) {
	var project, k = 150, x = 480, y = 250, dx, dy, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, projectRotate, theta = null, preclip = antimeridian_default, x0 = null, y0, x1, y1, postclip = identity_default$2, delta2 = .5, projectResample = resample_default(projectTransform, delta2), cache, cacheStream;
	...
	projection.clipExtent = function(_) {
		return arguments.length ? (postclip = _ == null ? (x0 = y0 = x1 = y1 = null, identity_default$2) : clipExtent(x0 = +_[0][0], y0 = +_[0][1], x1 = +_[1][0], y1 = +_[1][1]), reset()) : x0 == null ? null : [[x0, y0], [x1, y1]];
	};
...
//#endregion
//#region node_modules/@turf/buffer/dist/esm/index.js
var { BufferOp, GeoJSONReader, GeoJSONWriter } = import_jsts_min.default;
...
function bufferFeature(geojson, radius, units, steps) {
	...
	var geom = new GeoJSONReader().read(projected);
	var distance = radiansToLength(lengthToRadians(radius, units), "meters");
	var buffered = BufferOp.bufferOp(geom, distance, steps);
	buffered = new GeoJSONWriter().write(buffered);
...
//#endregion
//#region node_modules/@turf/center-mean/dist/esm/index.js
function centerMean(geojson, options = {}) {
	...
	geomEach(geojson, function(geom, featureIndex, properties) {
		...
		if (weight > 0) coordEach(geom, function(coord) {
...
//#endregion
//#region node_modules/@turf/center-median/dist/esm/index.js
function centerMedian(features, options = {}) {
	...
	featureEach(features, function(feature) {
		var _a;
		centroids.features.push(centroid(feature, { properties: { weight: (_a = feature.properties) == null ? void 0 : _a[weightTerm] } }));
	});
	const properties = {
...
function findMedian(candidateMedian, previousCandidate, centroids, properties, counter) {
	var tolerance = properties.tolerance || .001;
	var candidateXsum = 0;
	var candidateYsum = 0;
	var kSum = 0;
	var centroidCount = 0;
	...
	if (centroidCount === 1 || counter === 0 || Math.abs(candidateX - previousCandidate[0]) < tolerance && Math.abs(candidateY - previousCandidate[1]) < tolerance) return point([candidateX, candidateY], { medianCandidates: properties.medianCandidates });
	else {
...
var tinyqueue_exports = /* @__PURE__ */ __exportAll({ default: () => TinyQueue });
function defaultCompare(a, b) {
	return a < b ? -1 : a > b ? 1 : 0;
}
var TinyQueue;
var init_tinyqueue = __esmMin((() => {
	TinyQueue = class {
...
//#endregion
//#region node_modules/point-in-polygon/flat.js
var require_flat = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/point-in-polygon/nested.js
var require_nested = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/point-in-polygon/index.js
var require_point_in_polygon = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/robust-predicates/umd/orient2d.min.js
var require_orient2d_min = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(t, e) {
...
//#endregion
//#region node_modules/@turf/convex/dist/esm/index.js
var import_concaveman = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function noIntersections(a, b, segTree) {
		...
		for (var i = 0; i < edges.length; i++) if (intersects(edges[i].p, edges[i].next.p, a, b)) return false;
		return true;
	}
	...
	function fastConvexHull(points) {
		...
		for (i = 0; i < points.length; i++) if (!pointInPolygon(points[i], cull)) filtered.push(points[i]);
	...
	function convexHull(points) {
		...
		for (var ii = points.length - 1; ii >= 0; ii--) {
			while (upper.length >= 2 && cross(upper[upper.length - 2], upper[upper.length - 1], points[ii]) <= 0) upper.pop();
...
function convex(geojson, options = {}) {
	...
	if (convexHull.length > 3) return polygon([convexHull]);
	return null;
}
//#endregion
//#region node_modules/@turf/center-of-mass/dist/esm/index.js
function centerOfMass(geojson, options = {}) {
	switch (getType(geojson)) {
		case "Point": return point(getCoord(geojson), options.properties);
		case "Polygon":
			var coords = [];
			coordEach(geojson, function(coord) {
				coords.push(coord);
			...
			if (sArea === 0) return centre;
			else {
				var areaFactor = 1 / (6 * (sArea * .5));
...
//#endregion
//#region node_modules/@turf/circle/dist/esm/index.js
function circle(center, radius, options = {}) {
	...
	const coordinates = [];
	for (let i = 0; i < steps; i++) coordinates.push(destination(center, radius, i * -360 / steps, options).geometry.coordinates);
...
function clone(geojson) {
	if (!geojson) throw new Error("geojson is required");
	switch (geojson.type) {
		case "Feature": return cloneFeature(geojson);
		case "FeatureCollection": return cloneFeatureCollection(geojson);
		case "Point":
		case "LineString":
		case "Polygon":
		case "MultiPoint":
		case "MultiLineString":
		case "MultiPolygon":
		case "GeometryCollection": return cloneGeometry(geojson);
...
function cloneFeature(geojson) {
	...
	Object.keys(geojson).forEach((key) => {
		switch (key) {
			...
			default: cloned[key] = geojson[key];
		}
	});
	cloned.properties = cloneProperties(geojson.properties);
	if (geojson.geometry == null) cloned.geometry = null;
	else cloned.geometry = cloneGeometry(geojson.geometry);
	return cloned;
}
function cloneProperties(properties) {
	...
	Object.keys(properties).forEach((key) => {
		...
		if (typeof value === "object") if (value === null) cloned[key] = null;
		else if (Array.isArray(value)) cloned[key] = value.map((item) => {
			return item;
		});
		else cloned[key] = cloneProperties(value);
...
//#endregion
//#region node_modules/@turf/clusters/dist/esm/index.js
...
function clusterEach(geojson, property, callback) {
	...
	for (var index = 0; index < values.length; index++) {
		...
		var bin = bins[value];
		var features = [];
		for (var i = 0; i < bin.length; i++) features.push(geojson.features[bin[i]]);
...
function applyFilter(properties, filter) {
	...
	if (filterType === "number" || filterType === "string") return Object.prototype.hasOwnProperty.call(properties, filter);
		...
		for (var i = 0; i < filter.length; i++) if (!applyFilter(properties, filter[i])) return false;
		return true;
	} else return propertiesContainsFilter(properties, filter);
}
...
//#endregion
//#region node_modules/@turf/clusters-dbscan/dist/esm/index.js
function clustersDbscan(points, maxDistance, options = {}) {
	if (options.mutate !== true) points = clone(points);
	const minPoints = options.minPoints || 3;
	const latDistanceInDegrees = lengthToDegrees(maxDistance, options.units);
	var tree = new import_rbush_min.default(points.features.length);
	...
	const regionQuery = (index) => {
		...
		const lonDistanceInDegrees = function() {
			if (minY < 0 && maxY > 0) return latDistanceInDegrees;
			if (Math.abs(minY) < Math.abs(maxY)) return latDistanceInDegrees / Math.cos(degreesToRadians(maxY));
			else return latDistanceInDegrees / Math.cos(degreesToRadians(minY));
		}();
		const bbox = {
			minX: Math.max(x - lonDistanceInDegrees, -360),
			minY,
			maxX: Math.min(x + lonDistanceInDegrees, 360),
			maxY
		};
		return tree.search(bbox).filter((neighbor) => {
			const neighborIndex = neighbor.index;
			const neighborPoint = points.features[neighborIndex];
			return distance(point, neighborPoint, { units: "kilometers" }) <= maxDistance;
		});
	};
	const expandCluster = (clusteredId, neighbors) => {
		for (var i = 0; i < neighbors.length; i++) {
			...
			if (!visited[neighborIndex]) {
				...
				if (nextNeighbors.length >= minPoints) neighbors.push(...nextNeighbors);
			}
	...
	points.features.forEach((_, index) => {
		...
		} else clusterPoint.properties.dbscan = "noise";
	});
	return points;
}
//#endregion
//#region node_modules/skmeans/dist/node/distance.js
var require_distance = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = {
...
//#endregion
//#region node_modules/skmeans/dist/node/kinit.js
var require_kinit = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Distance = require_distance(), eudist = Distance.eudist, dist = Distance.dist;
	module.exports = {
...
//#endregion
//#region node_modules/@turf/clusters-kmeans/dist/esm/index.js
var import_main = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Distance = require_distance(), ClusterInit = require_kinit(), eudist = Distance.eudist;
	Distance.mandist;
	Distance.dist;
	var kmrand = ClusterInit.kmrand, kmpp = ClusterInit.kmpp;
	var MAX = 1e4;
	/**
	...
	*/
	...
	function skmeans(data, k, initial, maxit) {
		...
		} else if (initial == "kmrand") ks = kmrand(data, k);
		else if (initial == "kmpp") ks = kmpp(data, k);
		else ks = initial;
		do {
			init(k, 0, count);
			for (var i = 0; i < len; i++) {
			...
			if (multi) {
				...
				for (var _j4 = 0; _j4 < k; _j4++) ks[_j4] = sum[_j4] / count[_j4] || 0;
				conv = true;
...
function clustersKmeans(points, options = {}) {
	...
	var skmeansResult = (0, import_main.default)(data, options.numberOfClusters, initialCentroids);
	var centroids = {};
...
//#endregion
//#region node_modules/@turf/collect/dist/esm/index.js
function collect(polygons, points, inProperty, outProperty) {
	var rtree = new import_rbush_min.default(6);
	...
	polygons.features.forEach(function(poly) {
		...
		potentialPoints.forEach(function(pt) {
			if (booleanPointInPolygon([pt.minX, pt.minY], poly)) values.push(pt.property);
		});
...
//#endregion
//#region node_modules/@turf/combine/dist/esm/index.js
function combine(fc) {
	var groups = {
		MultiPoint: {
			coordinates: [],
...
//#endregion
//#region node_modules/@turf/tin/dist/esm/index.js
...
function triangulate(vertices) {
	...
	while (i--) {
		...
		while (j) {
			...
			if (Math.abs(G) > epsilon) open.push(new Triangle(a, b, c));
		}
	}
...
function transform_default(transform) {
	if (transform == null) return identity_default$1;
...
var buffer$1 = /* @__PURE__ */ new ArrayBuffer(16), floats = new Float64Array(buffer$1), uints = new Uint32Array(buffer$1);
...
function join_default(topology) {
	...
	function index() {
		var indexByPoint = hashmap_default(coordinates.length * 1.4, hashIndex, equalIndex, Int32Array, -1, Int32Array), indexes = new Int32Array(coordinates.length);
		for (var i = 0, n = coordinates.length; i < n; ++i) indexes[i] = indexByPoint.maybeSet(i, i);
		return indexes;
	}
	function hashIndex(i) {
		return point_hash_default(coordinates[i]);
	}
	function equalIndex(i, j) {
		return point_equal_default(coordinates[i], coordinates[j]);
	}
...
function cut_default(topology) {
	...
	for (i = 0, n = rings.length; i < n; ++i) {
		...
		while (++ringMid < ringEnd) if (junctions.has(coordinates[ringMid])) if (ringFixed) {
			next = {
				0: ringMid,
				1: ring[1]
			};
...
function dedup_default(topology) {
	...
	function equalRing(arcA, arcB) {
		...
		for (var i = 0; i < n; ++i) if (!point_equal_default(coordinates[ia + (i + ka) % n], coordinates[ib + (i + kb) % n])) return false;
		return true;
	}
	function reverseEqualRing(arcA, arcB) {
		...
		for (var i = 0; i < n; ++i) if (!point_equal_default(coordinates[ia + (i + ka) % n], coordinates[jb - (i + kb) % n])) return false;
		return true;
	}
...
//#endregion
//#region node_modules/@turf/concave/dist/esm/index.js
...
function polygonDissolve(geojson, options = {}) {
	if (getType(geojson) !== "FeatureCollection") throw new Error("geojson must be a FeatureCollection");
	if (!geojson.features.length) throw new Error("geojson is empty");
	if (options.mutate === false || options.mutate === void 0) geojson = clone(geojson);
	const geoms = [];
	...
	return merge_default(topo, topo.objects.geoms.geometries);
}
function dissolve$1(geojson, options = {}) {
	...
	const data = geojson;
	switch (type) {
		case "LineString": return lineDissolve(data, options);
		case "Polygon": return polygonDissolve(data, options);
...
function concave(points, options = {}) {
	...
	const tinPolys = tin(removeDuplicates(points));
	...
	const dissolved = dissolve$1(tinPolys);
...
var esm_exports$6 = /* @__PURE__ */ __exportAll({
...
var constant_default$1 = (x) => {
	return () => {
		return x;
	};
};
var compare_default = (eps) => {
	const almostEqual = eps ? (a, b) => b.minus(a).abs().isLessThanOrEqualTo(eps) : constant_default$1(false);
	return (a, b) => {
...
function orient_default(eps) {
	const almostCollinear = eps ? (area2, ax, ay, cx, cy) => area2.exponentiatedBy(2).isLessThanOrEqualTo(cx.minus(ax).exponentiatedBy(2).plus(cy.minus(ay).exponentiatedBy(2)).times(eps)) : constant_default$1(false);
	return (a, b, c) => {
...
var crossProduct$1 = (a, b) => a.x.times(b.y).minus(a.y.times(b.x));
...
var sineOfAngle = (pShared, pBase, pAngle) => {
	...
	return crossProduct$1(vAngle, vBase).div(length$1(vAngle)).div(length$1(vBase));
};
...
var intersection$1 = (pt1, v1, pt2, v2) => {
	...
	const kross = crossProduct$1(v1, v2);
	if (kross.isZero()) return null;
	const ve = {
		x: pt2.x.minus(pt1.x),
		y: pt2.y.minus(pt1.y)
	};
	const d1 = crossProduct$1(ve, v1).div(kross);
	const d2 = crossProduct$1(ve, v2).div(kross);
...
var Segment = class _Segment {
	...
	*/
	...
	beforeState() {
		...
		else this._beforeState = (this.prev.consumedBy || this.prev).afterState();
	...
	isInResult() {
		...
		switch (operation_default.type) {
			case "union":
				this._isInResult = mpsBefore.length === 0 !== (mpsAfter.length === 0);
				break;
			case "intersection": {
				let least;
				let most;
			...
			case "xor":
				this._isInResult = Math.abs(mpsBefore.length - mpsAfter.length) % 2 === 1;
				break;
			case "difference": {
...
//#endregion
//#region node_modules/@turf/dissolve/dist/esm/index.js
function dissolve(fc, options = {}) {
	...
	const outFeatures = [];
	if (!propertyName) return flatten(multiPolygon(union.apply(null, fc.features.map(function(f) {
		...
		featureEach(fc, function(feature) {
			if (feature.properties) {
				if (!Object.prototype.hasOwnProperty.call(uniquePropertyVals, feature.properties[propertyName])) uniquePropertyVals[feature.properties[propertyName]] = [];
				uniquePropertyVals[feature.properties[propertyName]].push(feature);
			}
		});
		const vals = Object.keys(uniquePropertyVals);
		for (let i = 0; i < vals.length; i++) {
			const mp = multiPolygon(union.apply(null, uniquePropertyVals[vals[i]].map(function(f) {
				return f.geometry.coordinates;
			})));
			if (mp && mp.properties) {
...
//#endregion
//#region node_modules/@turf/distance-weight/dist/esm/index.js
function pNormDistance(feature1, feature2, p = 2) {
	...
	if (p === 1) return Math.abs(xDiff) + Math.abs(yDiff);
...
function distanceWeight(fc, options) {
	...
	const weights = [];
	for (let i = 0; i < features.length; i++) weights[i] = [];
	for (let i = 0; i < features.length; i++) for (let j = i; j < features.length; j++) {
	...
	for (let i = 0; i < features.length; i++) for (let j = 0; j < features.length; j++) {
		...
		if (dis === 0) continue;
		if (binary) if (dis <= threshold) weights[i][j] = 1;
		else weights[i][j] = 0;
		else if (dis <= threshold) weights[i][j] = Math.pow(dis, alpha);
		else weights[i][j] = 0;
	}
	if (rowTransform) for (let i = 0; i < features.length; i++) {
		const rowSum = weights[i].reduce((sum, currentVal) => {
			return sum + currentVal;
		}, 0);
		for (let j = 0; j < features.length; j++) weights[i][j] = weights[i][j] / rowSum;
	}
	return weights;
}
//#endregion
...
function rhumbDistance(from, to, options = {}) {
	...
	return convertLength(calculateRhumbDistance(origin, destination), "meters", options.units);
}
...
function calculateRhumbDestination(origin, distance, bearing, radius) {
	...
	const DeltaPsi = Math.log(Math.tan(phi2 / 2 + Math.PI / 4) / Math.tan(phi1 / 2 + Math.PI / 4));
	const q = Math.abs(DeltaPsi) > 1e-11 ? DeltaPhi / DeltaPsi : Math.cos(phi1);
	return [((lambda1 + delta * Math.sin(theta) / q) * 180 / Math.PI + 540) % 360 - 180, phi2 * 180 / Math.PI];
}
//#endregion
...
function transformRotate(geojson, angle, options) {
	...
	coordEach(geojson, function(pointCoords) {
		const finalAngle = rhumbBearing(pivotCoord, pointCoords) + angle;
		const newCoords = getCoords(rhumbDestination(pivotCoord, rhumbDistance(pivotCoord, pointCoords), finalAngle));
...
//#endregion
//#region node_modules/@turf/ellipse/dist/esm/index.js
function ellipse(center, xSemiAxis, ySemiAxis, options) {
	...
	for (let i = 0; i < steps; i++) {
		x += w;
		if (m === 0) w = A / k / c;
		else w = (-(m * x + c) + Math.sqrt(Math.pow(m * x + c, 2) - 4 * (v * m) * -(A / k))) / (2 * (v * m));
		if (x != 0) quadrantParameters.push(x);
	}
	parameters.push(0);
	for (let i = 0; i < quadrantParameters.length; i++) parameters.push(quadrantParameters[i]);
	parameters.push(Math.PI / 2);
	for (let i = 0; i < quadrantParameters.length; i++) parameters.push(Math.PI - quadrantParameters[quadrantParameters.length - i - 1]);
	parameters.push(Math.PI);
	for (let i = 0; i < quadrantParameters.length; i++) parameters.push(Math.PI + quadrantParameters[i]);
	parameters.push(3 * Math.PI / 2);
	for (let i = 0; i < quadrantParameters.length; i++) parameters.push(2 * Math.PI - quadrantParameters[quadrantParameters.length - i - 1]);
	parameters.push(0);
	const coords = [];
	for (const param of parameters) {
		...
		coords.push(destination(centerCoords, r, angle + radiansToDegrees(theta), { units }).geometry.coordinates);
	}
	return polygon([coords], properties);
}
//#endregion
...
//#endregion
//#region node_modules/@turf/explode/dist/esm/index.js
function explode(geojson) {
	const points = [];
	if (geojson.type === "FeatureCollection") featureEach(geojson, function(feature) {
		coordEach(feature, function(coord) {
			points.push(point(coord, feature.properties));
		});
	});
	else if (geojson.type === "Feature") coordEach(geojson, function(coord) {
		points.push(point(coord, geojson.properties));
	});
	else coordEach(geojson, function(coord) {
		points.push(point(coord));
	});
	return featureCollection(points);
}
//#endregion
//#region node_modules/@turf/flip/dist/esm/index.js
...
//#endregion
//#region node_modules/arc/dist/esm/utils.js
/**
...
function roundCoords(coords) {
	...
	for (let i = 0; i < coords.length; i++) {
		...
		if (coord !== void 0) rounded[i] = Math.round((coord + Number.EPSILON) * MULTIPLIER) / MULTIPLIER;
	}
	return rounded;
}
/**
...
var R2D = 180 / Math.PI;
//#endregion
//#region node_modules/arc/dist/esm/coord.js
/**
...
var Coord = class Coord {
	...
	*/
	antipode() {
		...
		return new Coord(this.lon < 0 ? 180 + this.lon : (180 - this.lon) * -1, anti_lat);
	}
};
//#endregion
//#region node_modules/arc/dist/esm/arc.js
/**
	...
	json() {
		...
		if (this.geometries.length === 1) {
			...
			return {
				...
				geometry: {
					type: "LineString",
					coordinates: firstGeometry.coords
				},
				properties: this.properties
			};
		}
		return {
			...
			geometry: {
				type: "MultiLineString",
				coordinates: this.geometries.filter((geom) => geom !== void 0).map((geom) => geom.coords)
			},
			properties: this.properties
		};
	}
	...
	*/
	wkt() {
		...
		for (const geometry of this.geometries) {
			...
			if (coordStrings.length === 0) wktParts.push("LINESTRING EMPTY");
			else wktParts.push(`LINESTRING(${coordStrings.join(",")})`);
		}
		return wktParts.join("; ");
	}
};
//#endregion
//#region node_modules/arc/dist/esm/line-string.js
/**
...
//#endregion
//#region node_modules/arc/dist/esm/great-circle.js
/**
...
var GreatCircle = class {
	...
	constructor(start, end, properties) {
		if (!start || start.x === void 0 || start.y === void 0) throw new Error("GreatCircle constructor expects two args: start and end objects with x and y properties");
		if (!end || end.x === void 0 || end.y === void 0) throw new Error("GreatCircle constructor expects two args: start and end objects with x and y properties");
		...
		if (this.g === Math.PI) throw new Error("it appears " + this.start.view() + " and " + this.end.view() + " are 'antipodal', e.g diametrically opposite, thus there is no single route but rather infinite");
		else if (isNaN(this.g)) throw new Error("could not calculate great circle between " + start + " and " + end);
	}
	/**
	...
	*/
	Arc(npoints, options) {
		...
		for (let j = 1; j < first_pass.length; ++j) {
			...
			if (dfDiffLong > dfDiffSpace && (dfX > dfLeftBorderX && dfPrevX < dfRightBorderX || dfPrevX > dfLeftBorderX && dfX < dfRightBorderX)) bHasBigDiff = true;
			else if (dfDiffLong > dfMaxSmallDiffLong) dfMaxSmallDiffLong = dfDiffLong;
		}
		const poMulti = [];
		if (bHasBigDiff && dfMaxSmallDiffLong < dfDateLineOffset) {
			...
			for (let k = 0; k < first_pass.length; ++k) {
				...
				if (k > 0 && Math.abs(dfX0 - (first_pass[k - 1]?.[0] ?? 0)) > dfDiffSpace) {
					...
					if (dfX1 > dfLeftBorderX && dfX2 < dfRightBorderX) dfX2 + 360;
				...
				} else poNewLS.push([first_pass[k]?.[0] ?? 0, first_pass[k]?.[1] ?? 0]);
			}
		} else {
			const poNewLS0 = [];
			poMulti.push(poNewLS0);
			for (let l = 0; l < first_pass.length; ++l) poNewLS0.push([first_pass[l]?.[0] ?? 0, first_pass[l]?.[1] ?? 0]);
		}
		...
		for (let m = 0; m < poMulti.length; ++m) {
			...
			if (points) for (let j0 = 0; j0 < points.length; ++j0) {
				const point = points[j0];
				if (point) line.move_to(roundCoords([point[0], point[1]]));
			}
		}
		return arc;
	}
...
//#endregion
//#region node_modules/@turf/great-circle/dist/esm/index.js
function greatCircle(start, end, options = {}) {
	if (typeof options !== "object") throw new Error("options is invalid");
	const { properties = {}, npoints = 100, offset = 10 } = options;
	const startCoord = getCoord(start);
	const endCoord = getCoord(end);
	if (startCoord[0] === endCoord[0] && startCoord[1] === endCoord[1]) return lineString(Array(npoints).fill([startCoord[0], startCoord[1]]), properties);
	return new GreatCircle({
		x: startCoord[0],
		y: startCoord[1]
	}, {
		x: endCoord[0],
		y: endCoord[1]
	}, properties || {}).Arc(npoints, { offset }).json();
}
//#endregion
...
function intersect(features, options = {}) {
	...
	if (geoms.length < 2) throw new Error("Must specify at least 2 geometries");
	const intersection2$1 = intersection2(geoms[0], ...geoms.slice(1));
	if (intersection2$1.length === 0) return null;
	if (intersection2$1.length === 1) return polygon(intersection2$1[0], options.properties);
	return multiPolygon(intersection2$1, options.properties);
}
//#endregion
//#region node_modules/@turf/hex-grid/dist/esm/index.js
function hexGrid(bbox, cellSide, options = {}) {
	const clonedProperties = JSON.stringify(options.properties || {});
	const [west, south, east, north] = bbox;
	const centerY = (south + north) / 2;
	const centerX = (west + east) / 2;
	const cellWidth = cellSide * 2 / distance([west, centerY], [east, centerY], options) * (east - west);
	const cellHeight = cellSide * 2 / distance([centerX, south], [centerX, north], options) * (north - south);
	...
	if (hasOffsetY) y_adjust -= hex_height / 4;
	const cosines = [];
	const sines = [];
	for (let i = 0; i < 6; i++) {
	...
	const results = [];
	for (let x = 0; x <= x_count; x++) for (let y = 0; y <= y_count; y++) {
		const isOdd = x % 2 === 1;
		...
		if (isOdd) center_y -= hex_height / 2;
		if (options.triangles === true) hexTriangles([center_x, center_y], cellWidth / 2, cellHeight / 2, JSON.parse(clonedProperties), cosines, sines).forEach(function(triangle) {
			if (options.mask) {
				if (intersect(featureCollection([options.mask, triangle]))) results.push(triangle);
			} else results.push(triangle);
		});
		else {
...
//#endregion
//#region node_modules/@turf/point-grid/dist/esm/index.js
function pointGrid(bbox, cellSide, options = {}) {
	...
	var east = bbox[2];
	var north = bbox[3];
	var cellWidth = cellSide / distance([west, south], [east, south], options) * (east - west);
	var cellHeight = cellSide / distance([west, south], [west, north], options) * (north - south);
...
//#endregion
//#region node_modules/@turf/rectangle-grid/dist/esm/index.js
function rectangleGrid(bbox, cellWidth, cellHeight, options = {}) {
	...
	for (let column = 0; column < columns; column++) {
		...
		for (let row = 0; row < rows; row++) {
			...
			if (options.mask) {
				if (booleanIntersects(options.mask, cellPoly)) results.push(cellPoly);
			} else results.push(cellPoly);
...
//#endregion
//#region node_modules/@turf/square-grid/dist/esm/index.js
function squareGrid(bbox, cellSide, options = {}) {
	return rectangleGrid(bbox, cellSide, cellSide, options);
}
//#endregion
//#region node_modules/@turf/triangle-grid/dist/esm/index.js
function triangleGrid(bbox, cellSide, options = {}) {
	var results = [];
	var cellWidth = cellSide / distance([bbox[0], bbox[1]], [bbox[2], bbox[1]], options) * (bbox[2] - bbox[0]);
	var cellHeight = cellSide / distance([bbox[0], bbox[1]], [bbox[0], bbox[3]], options) * (bbox[3] - bbox[1]);
	var xi = 0;
	var currentX = bbox[0];
...
//#endregion
//#region node_modules/@turf/interpolate/dist/esm/index.js
function interpolate(points, cellSize, options) {
	...
	featureEach(grid, function(gridFeature) {
		...
		featureEach(points, function(point) {
			var _a3;
			var d = distance(gridType === "point" ? gridFeature : centroid(gridFeature), point, options);
			var zValue;
			if (property !== void 0) zValue = (_a3 = point.properties) == null ? void 0 : _a3[property];
...
//#endregion
//#region node_modules/@turf/isobands/dist/esm/index.js
...
function gridToMatrix$1(grid, options = {}) {
	...
	for (var r = 0; r < pointsMatrix.length; r++) {
		...
		for (var c = 0; c < pointRow.length; c++) {
			...
			if (point.properties == null) point.properties = {};
...
function sortPointsByLatLng$1(points, flip) {
	...
	const pointMatrix = [];
	for (const row of Object.values(pointsByLatitude)) pointMatrix.push(row.sort((a, b) => getCoords(a)[0] - getCoords(b)[0]));
	pointMatrix.sort(flip ? (a, b) => getCoords(a[0])[1] - getCoords(b[0])[1] : (a, b) => getCoords(b[0])[1] - getCoords(a[0])[1]);
	return pointMatrix;
}
function isobands(pointGrid, breaks, options) {
	...
	const matrix = gridToMatrix$1(pointGrid, {
		zProperty,
		flip: true
	});
	const dx = matrix[0].length;
	if (matrix.length < 2 || dx < 2) throw new Error("Matrix of points must be at least 2x2");
	for (let i = 1; i < matrix.length; i++) if (matrix[i].length !== dx) throw new Error("Matrix of points is not uniform in the x dimension");
	let contours = createContourLines(matrix, breaks, zProperty);
	contours = rescaleContours(contours, matrix, pointGrid);
	return featureCollection(contours.map((contour, index) => {
		if (breaksProperties[index] && !isObject(breaksProperties[index])) throw new Error("Each mappedProperty is required to be an Object");
		const contourProperties = __spreadValues$3(__spreadValues$3({}, commonProperties), breaksProperties[index]);
		contourProperties[zProperty] = contour[zProperty];
		return multiPolygon(contour.groupedRings, contourProperties);
	}));
}
function createContourLines(matrix, breaks, property) {
	...
	for (let i = 1; i < breaks.length; i++) {
		if (i === 1) prevSegments = getSegments(matrix, +breaks[0]);
		const upperBand = +breaks[i];
		const lowerBand = +breaks[i - 1];
		const segments = getSegments(matrix, upperBand);
		const reverseSegments = segments.map((segment) => segment.map((pos) => [pos[0], pos[1]]).reverse());
		const polygons = groupNestedRings(orderByArea(assembleRings(prevSegments.concat(reverseSegments), matrix)));
...
function getSegments(matrix, threshold) {
	...
	const dx = matrix[0].length;
	const dy = matrix.length;
	for (let y = 0; y < dy - 1; y++) for (let x = 0; x < dx - 1; x++) {
		const tr = matrix[y + 1][x + 1];
		const br = matrix[y][x + 1];
		const bl = matrix[y][x];
		const tl = matrix[y + 1][x];
		switch ((tl >= threshold ? 8 : 0) | (tr >= threshold ? 4 : 0) | (br >= threshold ? 2 : 0) | (bl >= threshold ? 1 : 0)) {
			case 0: continue;
			case 1:
			...
			case 4:
				...
				break;
			case 5:
				if ((tl + tr + br + bl) / 4 >= threshold) segments.push([[x + frac(tl, tr), y + 1], [x, y + frac(bl, tl)]], [[x + frac(bl, br), y], [x + 1, y + frac(br, tr)]]);
				else segments.push([[x + frac(tl, tr), y + 1], [x + 1, y + frac(br, tr)]], [[x + frac(bl, br), y], [x, y + frac(bl, tl)]]);
				break;
			case 6:
			...
			case 9:
				...
				break;
			case 10:
				if ((tl + tr + br + bl) / 4 >= threshold) segments.push([[x, y + frac(bl, tl)], [x + frac(bl, br), y]], [[x + 1, y + frac(br, tr)], [x + frac(tl, tr), y + 1]]);
				else segments.push([[x, y + frac(bl, tl)], [x + frac(tl, tr), y + 1]], [[x + 1, y + frac(br, tr)], [x + frac(bl, br), y]]);
				break;
			case 11:
...
function assembleRings(segments, matrix) {
	...
	while (contours.length > 0) {
		...
		let corner;
		if (end[0] === 0 && end[1] !== 0) {
			match = getAdjacentContour(contours, (contour2) => contour2[0][0] === 0 && contour2[0][1] < end[1], (a, b) => b[0][1] - a[0][1]);
			corner = [0, 0];
		} else if (end[1] === 0 && end[0] !== dx - 1) {
			match = getAdjacentContour(contours, (contour2) => contour2[0][1] === 0 && contour2[0][0] > end[0], (a, b) => a[0][0] - b[0][0]);
			corner = [dx - 1, 0];
		} else if (end[0] === dx - 1 && end[1] !== dy - 1) {
			match = getAdjacentContour(contours, (contour2) => contour2[0][0] === dx - 1 && contour2[0][1] > end[1], (a, b) => a[0][1] - b[0][1]);
			corner = [dx - 1, dy - 1];
		} else if (end[1] === dy - 1 && end[0] !== 0) {
			match = getAdjacentContour(contours, (contour2) => contour2[0][1] === dy - 1 && contour2[0][0] < end[0], (a, b) => b[0][0] - a[0][0]);
			corner = [0, dy - 1];
		} else throw new Error("Contour not closed but is not along an edge");
		if (match === -1) contour.push(corner);
		else if (match === 0) {
			...
			for (const p of matchedContour) contour.push(p);
		}
	}
	for (let i = 0; i < result.length; i++) if (result[i].length < 4) {
		result.splice(i, 1);
		i--;
	}
	return result;
...
function groupNestedRings(orderedLinearRings) {
	...
	while (!allGrouped(lrList)) for (let i = 0; i < lrList.length; i++) if (!lrList[i].grouped) {
		const group = [];
		group.push(lrList[i].lrCoordinates);
		lrList[i].grouped = true;
		const outerMostPoly = polygon([lrList[i].lrCoordinates]);
		OUTER: for (let j = i + 1; j < lrList.length; j++) if (!lrList[j].grouped) {
			const lrPoly = polygon([lrList[j].lrCoordinates]);
			if (isInside$1(lrPoly, outerMostPoly)) {
				for (let k = 1; k < group.length; k++) if (isInside$1(lrPoly, polygon([group[k]]))) continue OUTER;
...
function isInside$1(testPolygon, targetPolygon) {
	...
	for (let i = 0; i < points.features.length; i++) if (!booleanPointInPolygon(points.features[i], targetPolygon)) return false;
	return true;
}
function allGrouped(list) {
	for (let i = 0; i < list.length; i++) if (list[i].grouped === false) return false;
	return true;
}
function getAdjacentContour(contours, test, sort) {
	let match = -1;
	for (let j = 0; j < contours.length; j++) if (test(contours[j])) {
		if (match === -1 || sort(contours[match], contours[j]) > 0) match = j;
	}
	return match;
}
//#endregion
//#region node_modules/@turf/isolines/dist/esm/index.js
...
function isolines(pointGrid, breaks, options) {
	...
	if (matrix.length < 2 || dx < 2) throw new Error("Matrix of points must be at least 2x2");
	for (let i = 1; i < matrix.length; i++) if (matrix[i].length !== dx) throw new Error("Matrix of points is not uniform in the x dimension");
	return featureCollection(rescaleIsolines(createIsoLines(matrix, breaks, zProperty, commonProperties, breaksProperties), matrix, pointGrid));
}
function createIsoLines(matrix, breaks, zProperty, commonProperties, breaksProperties) {
	const results = [];
	for (let i = 0; i < breaks.length; i++) {
		const threshold = +breaks[i];
		const properties = __spreadValues$2(__spreadValues$2({}, commonProperties), breaksProperties[i]);
...
//#endregion
//#region node_modules/@turf/kinks/dist/esm/index.js
function kinks(featureIn) {
	...
	if (featureIn.type === "Feature") feature = featureIn.geometry;
	else feature = featureIn;
	if (feature.type === "LineString") coordinates = [feature.coordinates];
	else if (feature.type === "MultiLineString") coordinates = feature.coordinates;
	else if (feature.type === "MultiPolygon") coordinates = [].concat(...feature.coordinates);
	else if (feature.type === "Polygon") coordinates = feature.coordinates;
	...
	coordinates.forEach((line1) => {
		coordinates.forEach((line2) => {
			for (let i = 0; i < line1.length - 1; i++) for (let k = i; k < line2.length - 1; k++) {
				if (line1 === line2) {
					if (Math.abs(i - k) === 1) continue;
					if (i === 0 && k === line1.length - 2 && line1[i][0] === line1[line1.length - 1][0] && line1[i][1] === line1[line1.length - 1][1]) continue;
				}
				...
				if (intersection) results.features.push(point([intersection[0], intersection[1]]));
			}
		});
	});
	return results;
...
function lineIntersects(line1StartX, line1StartY, line1EndX, line1EndY, line2StartX, line2StartY, line2EndX, line2EndY) {
	...
	if (denominator === 0) if (result.x !== null && result.y !== null) return result;
	else return false;
	...
	if (result.onLine1 && result.onLine2) return [result.x, result.y];
	else return false;
}
//#endregion
//#region node_modules/@turf/length/dist/esm/index.js
...
//#endregion
//#region node_modules/@turf/line-arc/dist/esm/index.js
function lineArc(center, radius, bearing1, bearing2, options = {}) {
	...
	if (angle1 === angle2) return lineString(circle(center, radius, options).geometry.coordinates[0], properties);
...
//#endregion
//#region node_modules/@turf/line-slice-along/dist/esm/index.js
function lineSliceAlong(line, startDist, stopDist, options = {}) {
	...
	for (let i = 0; i < coords.length; i++) {
		...
		if (i === coords.length - 1) return lineString(slice);
...
//#endregion
//#region node_modules/@turf/line-chunk/dist/esm/index.js
...
function sliceLineSegments(line, segmentLength, units, callback) {
	...
	if (!Number.isInteger(numberOfSegments)) numberOfSegments = Math.floor(numberOfSegments) + 1;
	for (var i = 0; i < numberOfSegments; i++) callback(lineSliceAlong(line, segmentLength * i, segmentLength * (i + 1), { units }));
}
//#endregion
//#region node_modules/@turf/line-offset/dist/esm/index.js
function ab(segment) {
	var start = segment[0];
	var end = segment[1];
...
function intersectSegments(a, b) {
	...
	return add(p, scalarMult(crossProduct(sub(q, p), s) / cross, r));
}
function isParallel(a, b) {
	return crossProduct(ab(a), ab(b)) === 0;
}
...
function lineOffset(geojson, distance, options = {}) {
	...
	switch (type) {
		case "LineString": return lineOffsetFeature(geojson, distance, units);
		case "MultiLineString":
			var coords = [];
			flattenEach(geojson, function(feature) {
		...
		default: throw new Error("geometry " + type + " is not supported");
	}
}
function lineOffsetFeature(line, distance, units) {
	...
	return lineString(finalCoords, line.type === "Feature" ? line.properties : {});
}
...
//#endregion
//#region node_modules/@turf/line-slice/dist/esm/index.js
function lineSlice(startPt, stopPt, line) {
	...
	for (let i = ends[0].properties.segmentIndex + 1; i < ends[1].properties.segmentIndex + 1; i++) clipCoords.push(coords[i]);
...
function fixSegmentIndexBounds(line, vertex) {
	...
	if (vertex.properties.segmentIndex >= geometry.coordinates.length - 1) vertex.properties.segmentIndex = geometry.coordinates.length - 2;
}
//#endregion
//#region node_modules/@turf/line-to-polygon/dist/esm/index.js
function lineToPolygon(lines, options = {}) {
	var _a, _b, _c;
	var properties = options.properties;
	var autoComplete = (_a = options.autoComplete) != null ? _a : true;
	var orderCoords = (_b = options.orderCoords) != null ? _b : true;
	if (!((_c = options.mutate) != null ? _c : false)) lines = clone(lines);
	switch (lines.type) {
		case "FeatureCollection":
			var coords = [];
		...
		default: return lineStringToPolygon(lines, properties, autoComplete, orderCoords);
	}
}
function lineStringToPolygon(line, properties, autoComplete, orderCoords) {
	...
	switch (type) {
		...
		default: throw new Error("geometry type " + type + " is not supported");
	}
}
function autoCompleteCoords(coords) {
	...
	var x2 = last[0];
	var y2 = last[1];
	if (x1 !== x2 || y1 !== y2) coords.push(first);
	return coords;
}
...
//#endregion
//#region node_modules/@turf/mask/dist/esm/index.js
function mask(polygon, mask2, options) {
	...
	if (mask2 && mutate === false) maskTemplate = clone(mask2);
	...
	else if (polygon.type === "Feature") polygonOuters = createGeomFromPolygonClippingOutput(union(polygon.geometry.coordinates));
	else polygonOuters = createGeomFromPolygonClippingOutput(union(polygon.coordinates));
...
function unionFc(fc) {
	return createGeomFromPolygonClippingOutput(fc.features.length === 2 ? union(fc.features[0].geometry.coordinates, fc.features[1].geometry.coordinates) : union.apply(esm_exports$6, fc.features.map(function(f) {
		return f.geometry.coordinates;
	})));
}
...
function createMask(mask2) {
	...
	if (mask2) if (mask2.type === "Feature") coordinates = mask2.geometry.coordinates;
	else coordinates = mask2.coordinates;
...
//#endregion
//#region node_modules/@turf/moran-index/dist/esm/index.js
function moranIndex(fc, options) {
	...
	const weight = distanceWeight(fc, {
		...
		binary,
		p,
		standardization: (_b = options.standardization) != null ? _b : true,
		threshold
	});
	const y = [];
	featureEach(fc, (feature) => {
	...
	const vNorm = (n * n * s1 - n * s2 + 3 * (s0 * s0)) / ((n - 1) * (n + 1) * (s0 * s0)) - expectedMoranIndex * expectedMoranIndex;
	const stdNorm = Math.sqrt(vNorm);
	return {
		expectedMoranIndex,
		moranIndex: moranIndex2,
		stdNorm,
		zNorm: (moranIndex2 - expectedMoranIndex) / stdNorm
	};
}
function mean(y) {
	let sum = 0;
	for (const item of y) sum += item;
	return sum / y.length;
}
function variance(y) {
	const yMean = mean(y);
	let sum = 0;
	for (const item of y) sum += Math.pow(item - yMean, 2);
	return sum / y.length;
}
//#endregion
//#region node_modules/@turf/nearest-point/dist/esm/index.js
...
function nearestPoint(targetPoint, points, options = {}) {
	...
	return __spreadProps(__spreadValues$1({}, nearestPoint2), { properties: __spreadProps(__spreadValues$1({}, nearestPoint2.properties), {
		featureIndex: bestFeatureIndex,
		distanceToPoint: minDist
	}) });
}
//#endregion
//#region node_modules/@turf/nearest-neighbor-analysis/dist/esm/index.js
function nearestNeighborAnalysis(dataset, options) {
	...
	const observedMeanDistance = features.map((feature, index) => {
		return distance(feature, nearestPoint(feature, featureCollection(features.filter((f, i) => {
			return i !== index;
		}))).geometry.coordinates, { units });
	}).reduce((sum, value) => {
		return sum + value;
	}, 0) / n;
	...
	const variance = .26136 / Math.sqrt(n * populationDensity);
...
//#endregion
//#region node_modules/@turf/point-to-line-distance/dist/esm/index.js
...
function distanceToSegment(p, a, b, options) {
	if (options.method === "geodesic") return nearestPointOnLine(lineString([a, b]).geometry, p, { units: "degrees" }).properties.pointDistance;
	const v = [b[0] - a[0], b[1] - a[1]];
	const c1 = dot([p[0] - a[0], p[1] - a[1]], v);
	if (c1 <= 0) return rhumbDistance(p, a, { units: "degrees" });
	const c2 = dot(v, v);
	if (c2 <= c1) return rhumbDistance(p, b, { units: "degrees" });
	const b2 = c1 / c2;
	return rhumbDistance(p, [a[0] + b2 * v[0], a[1] + b2 * v[1]], { units: "degrees" });
}
function dot(u, v) {
	return u[0] * v[0] + u[1] * v[1];
}
//#endregion
//#region node_modules/@turf/nearest-point-to-line/dist/esm/index.js
...
function nearestPointToLine(points, line, options = {}) {
	...
	if (pt) pt.properties = __spreadValues(__spreadValues(__spreadValues({}, { dist }), pt.properties), properties);
	return pt;
}
function normalize$1(points) {
	const features = [];
	switch (points.geometry ? points.geometry.type : points.type) {
		case "GeometryCollection":
			geomEach(points, (geom) => {
				if (geom.type === "Point") features.push({
					type: "Feature",
					properties: {},
					geometry: geom
				});
...
//#endregion
//#region node_modules/@turf/planepoint/dist/esm/index.js
function planepoint(point, triangle) {
	const coord = getCoord(point);
	const outer = getGeom(triangle).coordinates[0];
...
//#endregion
//#region node_modules/@turf/point-on-feature/dist/esm/index.js
function pointOnFeature(geojson) {
	...
	while (!onSurface && i < fc.features.length) {
		...
		let onLine = false;
		if (geom.type === "Point") {
			if (cent.geometry.coordinates[0] === geom.coordinates[0] && cent.geometry.coordinates[1] === geom.coordinates[1]) onSurface = true;
			...
			if (booleanPointInPolygon(cent, geom)) onSurface = true;
		}
		i++;
	}
	if (onSurface) return cent;
	else {
		const vertices = featureCollection([]);
		for (let f = 0; f < fc.features.length; f++) vertices.features = vertices.features.concat(explode(fc.features[f]).features);
		return point(nearestPoint(cent, vertices).geometry.coordinates);
	}
}
function normalize(geojson) {
	if (geojson.type !== "FeatureCollection") {
		if (geojson.type !== "Feature") return featureCollection([feature(geojson)]);
		return featureCollection([geojson]);
	}
	return geojson;
}
function pointOnSegment(x, y, x1, y1, x2, y2) {
	return Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)) === Math.sqrt((x - x1) * (x - x1) + (y - y1) * (y - y1)) + Math.sqrt((x2 - x) * (x2 - x) + (y2 - y) * (y2 - y));
}
//#endregion
//#region node_modules/@turf/points-within-polygon/dist/esm/index.js
...
//#endregion
//#region node_modules/@turf/point-to-polygon-distance/dist/esm/index.js
...
//#endregion
//#region node_modules/@turf/polygonize/dist/esm/index.js
function mathSign(x) {
	return (x > 0) - (x < 0) || +x;
}
function orientationIndex(p1, p2, q) {
	const dx1 = p2[0] - p1[0], dy1 = p2[1] - p1[1], dx2 = q[0] - p2[0];
	return mathSign(dx1 * (q[1] - p2[1]) - dx2 * dy1);
}
	...
	sortOuterEdges() {
		if (!this.outerEdgesSorted) {
			this.outerEdges.sort((a, b) => {
				...
				if (det < 0) return 1;
				if (det > 0) return -1;
				return Math.pow(aNode.coordinates[0] - this.coordinates[0], 2) + Math.pow(aNode.coordinates[1] - this.coordinates[1], 2) - (Math.pow(bNode.coordinates[0] - this.coordinates[0], 2) + Math.pow(bNode.coordinates[1] - this.coordinates[1], 2));
			});
	...
	static findEdgeRingContaining(testEdgeRing, shellList) {
		...
		shellList.forEach((shell) => {
			...
			if (envelopeContains(tryEnvelope, testEnvelope)) {
				...
				let testPoint;
				for (const pt of testEdgeRingCoordinates) if (!shell.some((edge) => coordinatesEqual(pt, edge.from.coordinates))) testPoint = pt;
...
var Graph$1 = class _Graph {
	...
	*/
	_computeNextCWEdges(node) {
		if (typeof node === "undefined") Object.keys(this.nodes).forEach((id) => this._computeNextCWEdges(this.nodes[id]));
		else node.getOuterEdges().forEach((edge, i) => {
...
function polygonize(geoJson) {
	const graph = Graph$1.fromGeoJson(geoJson);
	graph.deleteDangles();
...
//#endregion
//#region node_modules/@turf/polygon-smooth/dist/esm/index.js
	...
	geomEach(inputPolys, function(geom, geomIndex, properties) {
		if (geom.type === "Polygon") {
			...
			for (let i = 0; i < iterations; i++) {
				...
				processPolygon$2(poly, tempOutput);
		...
		} else throw new Error("geometry is invalid, must be Polygon or MultiPolygon");
	});
	return featureCollection(outPolys);
}
function processPolygon$2(poly, tempOutput) {
	var previousCoord;
	var previousGeometryIndex;
	coordEach(poly, function(currentCoord, coordIndex, featureIndex, multiFeatureIndex, geometryIndex) {
		if (previousGeometryIndex !== geometryIndex) tempOutput.push([]);
		else {
			var p0x = previousCoord[0];
			var p0y = previousCoord[1];
			var p1x = currentCoord[0];
			var p1y = currentCoord[1];
			tempOutput[geometryIndex].push([.75 * p0x + .25 * p1x, .75 * p0y + .25 * p1y]);
			tempOutput[geometryIndex].push([.25 * p0x + .75 * p1x, .25 * p0y + .75 * p1y]);
		}
...
function processMultiPolygon(poly, tempOutput) {
	...
	coordEach(poly, function(currentCoord, coordIndex, featureIndex, multiFeatureIndex, geometryIndex) {
		if (previousMultiFeatureIndex !== multiFeatureIndex) tempOutput.push([[]]);
		else if (previousGeometryIndex !== geometryIndex) tempOutput[multiFeatureIndex].push([]);
		else {
			var p0x = previousCoord[0];
			var p0y = previousCoord[1];
			var p1x = currentCoord[0];
			var p1y = currentCoord[1];
			tempOutput[multiFeatureIndex][geometryIndex].push([.75 * p0x + .25 * p1x, .75 * p0y + .25 * p1y]);
			tempOutput[multiFeatureIndex][geometryIndex].push([.25 * p0x + .75 * p1x, .25 * p0y + .75 * p1y]);
		}
		previousCoord = currentCoord;
...
//#endregion
//#region node_modules/@turf/polygon-tangents/dist/esm/index.js
function polygonTangents(pt, polygon) {
	...
	let ltan = [];
	let eprev;
	const bbox$3 = bbox(polygon);
	let nearestPtIndex = 0;
	let nearest = null;
	if (pointCoords[0] > bbox$3[0] && pointCoords[0] < bbox$3[2] && pointCoords[1] > bbox$3[1] && pointCoords[1] < bbox$3[3]) {
		nearest = nearestPoint(pt, explode(polygon));
		nearestPtIndex = nearest.properties.featureIndex;
	}
	switch (getType(polygon)) {
		case "Polygon":
			...
			[rtan, ltan] = processPolygon$1(polyCoords[0], pointCoords, eprev, rtan, ltan);
			break;
		case "MultiPolygon":
			...
			polyCoords.forEach(function(ring) {
				[rtan, ltan] = processPolygon$1(ring[0], pointCoords, eprev, rtan, ltan);
			});
			break;
	}
	return featureCollection([point(rtan), point(ltan)]);
}
function processPolygon$1(polygonCoords, ptCoords, eprev, rtan, ltan) {
	for (let i = 0; i < polygonCoords.length; i++) {
		const currentCoords = polygonCoords[i];
		let nextCoordPair = polygonCoords[i + 1];
		if (i === polygonCoords.length - 1) nextCoordPair = polygonCoords[0];
		const enext = isLeft(currentCoords, nextCoordPair, ptCoords);
		if (eprev <= 0 && enext > 0) {
			if (!isBelow(ptCoords, currentCoords, rtan)) rtan = currentCoords;
		} else if (eprev > 0 && enext <= 0) {
			if (!isAbove(ptCoords, currentCoords, ltan)) ltan = currentCoords;
		}
		eprev = enext;
	}
	return [rtan, ltan];
...
//#endregion
//#region node_modules/@turf/projection/dist/esm/index.js
var esm_exports$4 = /* @__PURE__ */ __exportAll({
...
function convertToMercator(lonLat) {
	var D2R = Math.PI / 180, A = 6378137, MAXEXTENT = 20037508.342789244;
	var xy = [A * (Math.abs(lonLat[0]) <= 180 ? lonLat[0] : lonLat[0] - sign(lonLat[0]) * 360) * D2R, A * Math.log(Math.tan(Math.PI * .25 + .5 * lonLat[1] * D2R))];
...
function convertToWgs84(xy) {
	var R2D = 180 / Math.PI;
	var A = 6378137;
	return [xy[0] * R2D / A, (Math.PI * .5 - 2 * Math.atan(Math.exp(-xy[1] / A))) * R2D];
}
function sign(x) {
	return x < 0 ? -1 : x > 0 ? 1 : 0;
}
//#endregion
//#region node_modules/@turf/quadrat-analysis/dist/esm/index.js
function quadratAnalysis(pointFeatureSet, options) {
	...
	const quadrats = squareGrid(studyBbox, Math.sqrt(sizeOfArea / numOfPoints * 2), { units: "meters" }).features;
	const quadratIdDict = {};
	for (let i = 0; i < quadrats.length; i++) quadratIdDict[i] = {
		box: bbox(quadrats[i]),
		cnt: 0
	};
	let sumOfPoint = 0;
	for (const pt of points) for (const key of Object.keys(quadratIdDict)) {
	...
	for (let x = 0; x < maxCnt + 1; x++) {
		for (const key of Object.keys(quadratIdDict)) if (quadratIdDict[key].cnt === x) cumulativeObservedQuads += 1;
	...
	const criticalValue = K_TABLE[confidenceLevel] / Math.sqrt(numOfQuadrat);
	const result = {
		criticalValue,
		isRandom: true,
...
//#endregion
//#region node_modules/@turf/random/dist/esm/index.js
var esm_exports$5 = /* @__PURE__ */ __exportAll({
...
function checkBBox(bbox) {
	if (bbox == null) return;
	else if (Array.isArray(bbox)) validateBBox(bbox);
	else if (bbox.bbox != null) validateBBox(bbox.bbox);
}
function randomPoint(count, options = {}) {
	checkBBox(options.bbox);
	if (count === void 0 || count === null) count = 1;
	const features = [];
	for (let i = 0; i < count; i++) features.push(point(randomPositionUnchecked(options.bbox)));
	return featureCollection(features);
}
function randomPolygon(count, options = {}) {
	checkBBox(options.bbox);
	if (count === void 0 || count === null) count = 1;
	if (options.bbox === void 0 || options.bbox === null) options.bbox = [
		-180,
		-90,
		180,
		90
	];
	if (!isNumber(options.num_vertices) || options.num_vertices === void 0) options.num_vertices = 10;
	if (!isNumber(options.max_radial_length) || options.max_radial_length === void 0) options.max_radial_length = 10;
	...
	if (options.max_radial_length > maxRadius) throw new Error("max_radial_length is greater than the radius of the bbox");
	const paddedBbox = [
...
function randomLineString(count, options = {}) {
	...
	if (count === void 0 || count === null) count = 1;
	if (!isNumber(num_vertices) || num_vertices === void 0 || num_vertices < 2) num_vertices = 10;
	if (!isNumber(max_length) || max_length === void 0) max_length = 1e-4;
	if (!isNumber(max_rotation) || max_rotation === void 0) max_rotation = Math.PI / 8;
	const features = [];
	for (let i = 0; i < count; i++) {
		const vertices = [randomPositionUnchecked(bbox)];
		for (let j = 0; j < num_vertices - 1; j++) {
			const angle = (j === 0 ? Math.random() * 2 * Math.PI : Math.tan((vertices[j][1] - vertices[j - 1][1]) / (vertices[j][0] - vertices[j - 1][0]))) + (Math.random() - .5) * max_rotation * 2;
...
function rewind(geojson, options = {}) {
	...
	if (!mutate && geojson.type !== "Point" && geojson.type !== "MultiPoint") geojson = clone(geojson);
	const results = [];
	switch (geojson.type) {
		case "GeometryCollection":
			geomEach(geojson, function(geometry) {
...
function rewindFeature(geojson, reverse) {
	switch (geojson.type === "Feature" ? geojson.geometry.type : geojson.type) {
		case "GeometryCollection":
			geomEach(geojson, function(geometry) {
				rewindFeature(geometry, reverse);
			});
		...
		case "MultiPolygon":
			...
			return geojson;
		case "Point":
		case "MultiPoint": return geojson;
	}
}
...
function rewindPolygon(coords, reverse) {
	if (booleanClockwise(coords[0]) !== reverse) coords[0].reverse();
	for (let i = 1; i < coords.length; i++) if (booleanClockwise(coords[i]) === reverse) coords[i].reverse();
}
//#endregion
//#region node_modules/@turf/sample/dist/esm/index.js
function sample(fc, num) {
	...
	return featureCollection(getRandomSubarray(fc.features, num));
}
...
//#endregion
//#region node_modules/@turf/sector/dist/esm/index.js
function sector(center, radius, bearing1, bearing2, options = {}) {
	...
	if (typeof options !== "object") throw new Error("options must be an object");
	if (convertAngleTo360(bearing1) === convertAngleTo360(bearing2)) return circle(center, radius, options);
...
//#endregion
//#region node_modules/@turf/transform-scale/dist/esm/index.js
...
function scale(feature, factor, origin) {
	...
	coordEach(feature, function(coord) {
		...
		const newCoord = getCoords(rhumbDestination(originCoord, originalDistance * factor, bearing));
		coord[0] = newCoord[0];
		coord[1] = newCoord[1];
...
function defineOrigin(geojson, origin) {
	...
	const bbox$2 = geojson.bbox ? geojson.bbox : bbox(geojson, { recompute: true });
	const west = bbox$2[0];
	const south = bbox$2[1];
	const east = bbox$2[2];
	const north = bbox$2[3];
	switch (origin) {
		case "sw":
		case "southwest":
		case "westsouth":
		case "bottomleft": return point([west, south]);
		case "se":
		case "southeast":
		case "eastsouth":
		case "bottomright": return point([east, south]);
		case "nw":
		case "northwest":
		case "westnorth":
		case "topleft": return point([west, north]);
		case "ne":
		case "northeast":
		case "eastnorth":
		case "topright": return point([east, north]);
		case "center": return center(geojson);
		case void 0:
		case null:
		case "centroid": return centroid(geojson);
		default: throw new Error("invalid origin");
	}
}
//#endregion
//#region node_modules/@turf/shortest-path/dist/esm/index.js
	...
	bubbleUp(n) {
		...
		while (true) {
			...
			if (child2N < length) {
				...
				if (this.scoreFunction(child2) < (swap === null ? elemScore : child1Score)) swap = child2N;
			}
			if (swap !== null) {
...
function shortestPath(start, end, options = {}) {
	...
	} else if (obstacles.type === "Feature" && obstacles.geometry.type === "Polygon") obstacles = featureCollection([obstacles]);
	else if (obstacles.type === "Polygon") obstacles = featureCollection([feature(getGeom(obstacles))]);
	else throw new Error("invalid obstacles");
	const collection = obstacles;
	collection.features.push(start);
	collection.features.push(end);
	const [west, south, east, north] = bbox(transformScale(bboxPolygon(bbox(collection)), 1.15));
	collection.features.pop();
	collection.features.pop();
	const columnsWithFraction = distance([west, south], [east, south], options) / resolution;
	const cellWidth = (east - west) / columnsWithFraction;
	const rowsWithFraction = distance([west, south], [west, north], options) / resolution;
...
function isInside(pt, polygons) {
	for (let i = 0; i < polygons.features.length; i++) if (booleanPointInPolygon(pt, polygons.features[i])) return true;
	return false;
}
//#endregion
//#region node_modules/@turf/simplify/dist/esm/index.js
function getSqDist(p1, p2) {
	var dx = p1[0] - p2[0], dy = p1[1] - p2[1];
	return dx * dx + dy * dy;
}
...
function simplifyGeom(geometry, tolerance, highQuality) {
	...
	if (type !== "GeometryCollection") switch (type) {
		case "LineString":
		...
		case "Polygon":
			...
			break;
		case "MultiPolygon": geometry.coordinates = geometry.coordinates.map((rings) => simplifyPolygon(rings, tolerance, highQuality));
	}
	return geometry;
}
function simplifyPolygon(coordinates, tolerance, highQuality) {
	return coordinates.map(function(ring) {
		if (ring.length < 4) throw new Error("invalid polygon");
		...
		while (!checkValidity(simpleRing) && ringTolerance >= Number.EPSILON) {
			ringTolerance -= ringTolerance * .01;
			simpleRing = simplify(ring, ringTolerance, highQuality);
		}
		if (!checkValidity(simpleRing)) return ring;
...
//#endregion
//#region node_modules/@turf/square/dist/esm/index.js
function square(bbox) {
	var west = bbox[0];
	var south = bbox[1];
	var east = bbox[2];
	var north = bbox[3];
	if (distance(bbox.slice(0, 2), [east, south]) >= distance(bbox.slice(0, 2), [west, north])) {
...
//#endregion
//#region node_modules/@turf/standard-deviational-ellipse/dist/esm/index.js
...
//#endregion
//#region node_modules/@turf/tag/dist/esm/index.js
...
//#endregion
//#region node_modules/@turf/tesselate/dist/esm/index.js
...
function tesselate(poly) {
	if (!poly.geometry || poly.geometry.type !== "Polygon" && poly.geometry.type !== "MultiPolygon") throw new Error("input must be a Polygon or MultiPolygon");
	const fc = {
		type: "FeatureCollection",
		features: []
	};
	if (poly.geometry.type === "Polygon") fc.features = processPolygon(poly.geometry.coordinates);
	else poly.geometry.coordinates.forEach(function(coordinates) {
...
function processPolygon(coordinates) {
	...
	result.forEach(function(vert, i2) {
		...
		if (data.vertices[index * dim + 2] !== void 0) vertices.push([
			data.vertices[index * dim],
			data.vertices[index * dim + 1],
			data.vertices[index * dim + 2]
		]);
		else vertices.push([data.vertices[index * dim], data.vertices[index * dim + 1]]);
	});
...
function flattenCoords(data) {
	...
	for (let i = 0; i < data.length; i++) {
		for (let j = 0; j < data[i].length; j++) for (let d = 0; d < dim; d++) result.vertices.push(data[i][j][d]);
		if (i > 0) {
			holeIndex += data[i - 1].length;
			result.holes.push(holeIndex);
		}
...
//#endregion
//#region node_modules/@turf/transform-translate/dist/esm/index.js
...
//#endregion
//#region node_modules/@turf/unkink-polygon/dist/esm/index.js
function geojsonPolygonSelfIntersections(feature, filterFn) {
	...
	var tree = new import_rbush_min.default();
	tree.load(allEdgesAsRbushTreeItems);
	for (var ringA = 0; ringA < coord.length; ringA++) for (var edgeA = 0; edgeA < coord[ringA].length - 1; edgeA++) tree.search(rbushTreeItem(ringA, edgeA)).forEach(function(bboxIsect) {
	...
	function ifIsectAddToOutput(ring02, edge02, ring1, edge1) {
		...
		var frac0;
		var frac1;
		if (end0[0] !== start0[0]) frac0 = (isect[0] - start0[0]) / (end0[0] - start0[0]);
		else frac0 = (isect[1] - start0[1]) / (end0[1] - start0[1]);
		if (end1[0] !== start1[0]) frac1 = (isect[0] - start1[0]) / (end1[0] - start1[0]);
		else frac1 = (isect[1] - start1[1]) / (end1[1] - start1[1]);
		if (frac0 >= 1 || frac0 <= 0 || frac1 >= 1 || frac1 <= 0) return;
		var key = isect;
		var unique = !seen[key.toString()];
		if (unique) seen[key.toString()] = true;
		if (filterFn) output.push(filterFn(isect, ring02, edge02, start0, end0, frac0, ring1, edge1, start1, end1, frac1, unique));
	}
...
function intersect$1(start0, end0, start1, end1) {
	...
	return [((x0 * y1 - y0 * x1) * (x2 - x3) - (x0 - x1) * (x2 * y3 - y2 * x3)) / denom, ((x0 * y1 - y0 * x1) * (y2 - y3) - (y0 - y1) * (x2 * y3 - y2 * x3)) / denom];
}
...
function simplepolygon(feature) {
	...
	if (numSelfIsect == 0) {
		...
		for (var i = 0; i < numRings; i++) outputFeatureArray.push(polygon([feature.geometry.coordinates[i]], {
			parent: -1,
	...
	var isectRbushTree = new import_rbush_min.default();
	...
	for (var i = 0; i < pseudoVtxListByRingAndEdge.length; i++) for (var j = 0; j < pseudoVtxListByRingAndEdge[i].length; j++) for (var k = 0; k < pseudoVtxListByRingAndEdge[i][j].length; k++) {
		let coordToFind;
		if (k == pseudoVtxListByRingAndEdge[i][j].length - 1) coordToFind = pseudoVtxListByRingAndEdge[i][modulo(j + 1, feature.geometry.coordinates[i].length - 1)][0].coord;
		else coordToFind = pseudoVtxListByRingAndEdge[i][j][k + 1].coord;
	...
	while (queue.length > 0) {
		...
		while (!equalArrays2(isectList[startIsect].coord, isectList[nxtIsect].coord)) {
			...
			if (nxtIsectInQueue != void 0) queue.splice(nxtIsectInQueue, 1);
	...
	function determineParents(output2) {
		...
		for (var i2 = 0; i2 < output2.features.length; i2++) if (output2.features[i2].properties.parent == -1) featuresWithoutParent.push(i2);
		if (featuresWithoutParent.length > 1) for (var i2 = 0; i2 < featuresWithoutParent.length; i2++) {
			...
			for (var j2 = 0; j2 < output2.features.length; j2++) {
				...
				if (booleanPointInPolygon(output2.features[featuresWithoutParent[i2]].geometry.coordinates[0][0], output2.features[j2], { ignoreBoundary: true })) {
					if (area(output2.features[j2]) < parentArea) parent = j2;
				}
			}
	...
	function setNetWinding(output2) {
		for (var i2 = 0; i2 < output2.features.length; i2++) if (output2.features[i2].properties.parent == -1) {
	...
	function setNetWindingOfChildren(output2, parent, ParentNetWinding) {
		for (var i2 = 0; i2 < output2.features.length; i2++) if (output2.features[i2].properties.parent == parent) {
...
function isConvex(pts, righthanded) {
	...
	return (pts[1][0] - pts[0][0]) * (pts[2][1] - pts[0][1]) - (pts[1][1] - pts[0][1]) * (pts[2][0] - pts[0][0]) >= 0 == righthanded;
}
...
//#endregion
//#region node_modules/d3-voronoi/src/constant.js
...
//#endregion
//#region node_modules/d3-voronoi/src/point.js
function x(d) {
	return d[0];
}
function y(d) {
...
//#endregion
//#region node_modules/d3-voronoi/src/RedBlackTree.js
...
RedBlackTree.prototype = {
	...
	remove: function(node) {
		...
		if (parent) if (parent.L === node) parent.L = next;
		else parent.R = next;
		else this._ = next;
		if (left && right) {
			red = next.C;
...
function RedBlackRotateLeft(tree, node) {
	...
	if (parent) if (parent.L === p) parent.L = q;
	else parent.R = q;
	else tree._ = q;
	q.U = parent;
	p.U = q;
...
//#endregion
//#region node_modules/d3-voronoi/src/Edge.js
...
function clipEdge(edge, x0, y0, x1, y1) {
	var a = edge[0], b = edge[1], ax = a[0], ay = a[1], bx = b[0], by = b[1], t0 = 0, t1 = 1, dx = bx - ax, dy = by - ay, r = x0 - ax;
	if (!dx && r > 0) return;
	r /= dx;
	if (dx < 0) {
		if (r < t0) return;
...
function clipEdges(x0, y0, x1, y1) {
	...
	while (i--) if (!connectEdge(edge = edges[i], x0, y0, x1, y1) || !clipEdge(edge, x0, y0, x1, y1) || !(Math.abs(edge[0][0] - edge[1][0]) > 1e-6 || Math.abs(edge[0][1] - edge[1][1]) > 1e-6)) delete edges[i];
}
//#endregion
//#region node_modules/d3-voronoi/src/Cell.js
...
function sortCellHalfedges() {
	for (var i = 0, n = cells.length, cell, halfedges, j, m; i < n; ++i) if ((cell = cells[i]) && (m = (halfedges = cell.halfedges).length)) {
...
function clipCells(x0, y0, x1, y1) {
	...
	for (iCell = 0; iCell < nCells; ++iCell) if (cell = cells[iCell]) {
		...
		while (iHalfedge < nHalfedges) {
			...
			if (Math.abs(endX - startX) > 1e-6 || Math.abs(endY - startY) > 1e-6) {
				halfedges.splice(iHalfedge, 0, edges.push(createBorderEdge(site, end, Math.abs(endX - x0) < 1e-6 && y1 - endY > 1e-6 ? [x0, Math.abs(startX - x0) < 1e-6 ? startY : y1] : Math.abs(endY - y1) < 1e-6 && x1 - endX > 1e-6 ? [Math.abs(startY - y1) < 1e-6 ? startX : x1, y1] : Math.abs(endX - x1) < 1e-6 && endY - y0 > 1e-6 ? [x1, Math.abs(startX - x1) < 1e-6 ? startY : y0] : Math.abs(endY - y0) < 1e-6 && endX - x0 > 1e-6 ? [Math.abs(startY - y0) < 1e-6 ? startX : x0, y0] : null)) - 1);
				++nHalfedges;
			}
		}
	...
	if (cover) {
		...
		for (iCell = 0, cover = null; iCell < nCells; ++iCell) if (cell = cells[iCell]) {
			site = cell.site;
			dx = site[0] - x0;
			dy = site[1] - y0;
			d2 = dx * dx + dy * dy;
	...
	for (iCell = 0; iCell < nCells; ++iCell) if (cell = cells[iCell]) {
		if (!cell.halfedges.length) delete cells[iCell];
	}
}
//#endregion
//#region node_modules/d3-voronoi/src/Circle.js
var circlePool = [];
var firstCircle;
function Circle() {
	RedBlackNode(this);
...
function attachCircle(arc) {
	...
	while (node) if (circle.y < node.y || circle.y === node.y && circle.x <= node.x) if (node.L) node = node.L;
	else {
		before = node.P;
		break;
	}
...
//#endregion
//#region node_modules/d3-voronoi/src/Beach.js
var beachPool = [];
function Beach() {
	RedBlackNode(this);
...
function removeBeach(beach) {
	...
	detachBeach(beach);
	var lArc = previous;
	while (lArc.circle && Math.abs(x - lArc.circle.x) < 1e-6 && Math.abs(y - lArc.circle.cy) < 1e-6) {
		previous = lArc.P;
	...
	detachCircle(lArc);
	var rArc = next;
	while (rArc.circle && Math.abs(x - rArc.circle.x) < 1e-6 && Math.abs(y - rArc.circle.cy) < 1e-6) {
		next = rArc.N;
...
//#endregion
//#region node_modules/d3-voronoi/src/Diagram.js
var epsilon = 1e-6;
var beaches;
var cells;
var circles;
...
//#endregion
//#region node_modules/d3-voronoi/src/voronoi.js
function voronoi_default() {
	...
	voronoi.x = function(_) {
		return arguments.length ? (x$1 = typeof _ === "function" ? _ : constant_default(+_), voronoi) : x$1;
	};
	voronoi.y = function(_) {
		return arguments.length ? (y$1 = typeof _ === "function" ? _ : constant_default(+_), voronoi) : y$1;
	};
...
//#endregion
//#region node_modules/@turf/voronoi/dist/esm/index.js
...
function voronoi2(points, options) {
	...
	return featureCollection(voronoi_default().x((feature) => feature.geometry.coordinates[0]).y((feature) => feature.geometry.coordinates[1]).extent([[bbox[0], bbox[1]], [bbox[2], bbox[3]]]).polygons(points.features).map(function(coords, index) {
		return Object.assign(coordsToPolygon(coords), { properties: cloneProperties(points.features[index].properties) });
	}));
}
//#endregion
//#region node_modules/@turf/directional-mean/dist/esm/index.js
function directionalMean(lines, options = {}) {
	...
	let sumOfLen = 0;
	const centroidList = [];
	if (isSegment) segmentEach(lines, (currentSegment) => {
		...
		if (isNaN(sin1) || isNaN(cos1)) return;
		else {
			sigmaSin += sin1;
			sigmaCos += cos1;
			countOfLines += 1;
	...
	else featureEach(lines, (currentFeature) => {
		if (currentFeature.geometry.type !== "LineString") throw new Error("shold to support MultiLineString?");
	...
	const [averageX, averageY] = getCoord(centroid(featureCollection(centroidList)));
	let meanLinestring;
	if (isPlanar) meanLinestring = getMeanLineString([averageX, averageY], cartesianAngle, averageLength, isPlanar);
...
function getLengthOfLineString(line, isPlanar) {
	if (isPlanar) return segmentReduce(line, (previousValue, segment) => {
		const coords = segment.geometry.coordinates;
		return previousValue + euclideanDistance(coords);
	}, 0);
	else return length(line, { units: "meters" });
}
...
function getCosAndSin(coordinates, isPlanar) {
	...
	if (isPlanar) {
		...
		if (h < 1e-9) return [NaN, NaN];
		return [dy / h, dx / h];
	} else {
		const radian = bearingToCartesian(bearing(beginPoint, endPoint)) * Math.PI / 180;
		return [Math.sin(radian), Math.cos(radian)];
	}
}
function getAngleBySinAndCos(sin1, cos1) {
	let angle = 0;
	if (Math.abs(cos1) < 1e-9) angle = 90;
	else angle = Math.atan2(sin1, cos1) * 180 / Math.PI;
	if (sin1 >= 0) {
		if (cos1 < 0) angle += 180;
	} else if (cos1 < 0) angle -= 180;
	return angle;
}
function getCircularVariance(sin1, cos1, len) {
	if (len === 0) throw new Error("the size of the features set must be greater than 0");
...
function getMeanLineString(centroidOfLine, angle, lenOfLine, isPlanar) {
	if (isPlanar) {
		...
		return [getCoord(destination(point(centroidOfLine), -lenOfLine / 2, angle, { units: "meters" })), getCoord(end)];
	}
}
//#endregion
export { along, angle, applyFilter, area, areaFactors, azimuthToBearing, bbox, bboxClip, bboxPolygon, bearing, bearingToAzimuth, bezierSpline, booleanClockwise, booleanConcave, booleanContains, booleanCrosses, booleanDisjoint, booleanEqual, booleanIntersects, booleanOverlap, booleanParallel, booleanPointInPolygon, booleanPointOnLine, booleanTouches, booleanValid, booleanWithin, buffer, center, centerMean, centerMedian, centerOfMass, centroid, circle, cleanCoords, clone, cloneProperties, clusterEach, clusterReduce, esm_exports as clusters, clustersDbscan, clustersKmeans, collect, collectionOf, combine, concave, containsNumber, convertArea, convertLength, convex, coordAll, coordEach, coordReduce, createBins, degreesToRadians, destination, difference2 as difference, directionalMean, dissolve, distance, distanceWeight, earthRadius, ellipse, envelope, explode, factors, feature, featureCollection, featureEach, featureOf, featureReduce, filterProperties, findPoint, findSegment, flatten, flattenEach, flattenReduce, flip, geojsonRbush, geojsonType, geomEach, geomReduce, geometry, geometryCollection, getCluster, getCoord, getCoords, getGeom, getType, greatCircle, esm_exports$1 as helpers, hexGrid, interpolate, intersect, esm_exports$2 as invariant, isNumber, isObject, isobands, isolines, kinks, length, lengthToDegrees, lengthToRadians, lineArc, lineChunk, lineEach, lineIntersect, lineOffset, lineOverlap, lineReduce, lineSegment, lineSlice, lineSliceAlong, lineSplit, lineString, lineStrings, lineToPolygon, mask, esm_exports$3 as meta, midpoint, moranIndex, multiLineString, multiPoint, multiPolygon, nearestNeighborAnalysis, nearestPoint, nearestPointOnLine, nearestPointToLine, planepoint, point, pointGrid, pointOnFeature, pointToLineDistance, pointToPolygonDistance, points, pointsWithinPolygon, polygon, polygonSmooth, polygonTangents, polygonToLine, polygonize, polygons, esm_exports$4 as projection, propEach, propReduce, propertiesContainsFilter, quadratAnalysis, radiansToDegrees, radiansToLength, esm_exports$5 as random, randomLineString, randomPoint, randomPolygon, randomPosition, rectangleGrid, rewind, rhumbBearing, rhumbDestination, rhumbDistance, round, sample, sector, segmentEach, segmentReduce, shortestPath, simplify2 as simplify, square, squareGrid, standardDeviationalEllipse, tag, tesselate, tin, toMercator, toWgs84, transformRotate, transformScale, transformTranslate, triangleGrid, truncate, union2 as union, unkinkPolygon, validateBBox, validateId, voronoi2 as voronoi };
